...
import type { SiteTranslations } from '@/lib/translations/types';

export let lang: string = 'en';
// Strings resolved server-side and passed down through Header. Importing
// getTranslations here would bundle every locale file into the client JS.
export let nav: SiteTranslations['nav'];
...
{#if open}
  <div class="fixed inset-0 z-50 bg-main text-white flex flex-col items-center justify-start pt-20 gap-6 overflow-y-auto overscroll-contain transition-all duration-300 md:hidden">
    ...
    <a href="{prefix}/developers" class="masthead-link text-xl text-center" on:click={() => trackEvent(EVENTS.NAV_CLICK, { item: 'developers', source: 'mobile' })}>{nav.developers}</a>
    <button
    ...
    {#if repoOpen}
      ...
      >
        <a href={REPO_URLS.website} target="_blank" rel="noopener noreferrer" class="masthead-link text-base sm:text-lg text-center py-1 transition" on:click={() => trackEvent(EVENTS.NAV_CLICK, { item: 'github', repo: 'website', source: 'mobile' })}>{nav.repo.website}</a>
        <a href={REPO_URLS.skill} target="_blank" rel="noopener noreferrer" class="masthead-link text-base sm:text-lg text-center py-1 transition" on:click={() => trackEvent(EVENTS.NAV_CLICK, { item: 'github', repo: 'skill', source: 'mobile' })}>{nav.repo.skill}</a>
      </div>
    {/if}
    <button
      class="masthead-link text-xl text-center flex items-center justify-center gap-2 focus:outline-none cursor-pointer"
