...
O Deep Work Plan é a disciplina de fazer esse trabalho de montante de forma duradoura e repetível. Seus dois pilares fundacionais são as duas metades de uma boa direção: uma **especificação** declara como é o "correto" e um **harness** dá ao agente o contexto e as ferramentas para alcançá-lo. Juntos, eles convertem a habilidade bruta de um modelo em engenharia confiável — sustentada ao longo de uma tarefa que dura horas e preservada entre agentes que mudam de uma sessão para outra. Uma terceira disciplina mantém a combinação acessível em escala: a **eficiência de tokens**, porque um harness que funciona é um que o agente pode carregar por um horizonte longo.

## Eficiente por construção

O primeiro pilar torna o trabalho correto; o segundo o torna portátil. O terceiro o mantém **acessível** — porque o contexto é o recurso mais escasso que um agente de código tem, e uma metodologia que o desperdiça falha exatamente quando o trabalho se alonga.

Três mecanismos fazem o trabalho:

- **Divulgação progressiva.** O agente lê um núcleo essencial, e cada guia ou seção da especificação é carregado apenas quando o próprio fluxo dispara o gatilho. O agente carrega o capítulo de que precisa, não o livro inteiro.
- **Validação selecionada pela superfície tocada.** Cada tarefa registra o que muda e escolhe seu portão pela classe de risco — uma mudança pequena e isolada se prova de forma delimitada; uma mudança em maquinaria compartilhada se prova amplamente.
- **Aprendizado local por tarefa.** Quando uma tarefa produz um padrão reutilizável, a decisão acontece dentro dela, onde a evidência está fresca — não em uma retrospectiva que ninguém lê.

Nada disso afirma ser sem perdas por construção, nem é promovido com porcentagens inventadas. Os bytes de instrução que cada fluxo exige são medidos e publicados; as afirmações comportamentais são respaldadas por replays com falhas semeadas; e uma avaliação com agentes novos — os mesmos dois recursos construídos sem harness, com a versão maior anterior e com esta — já foi executada sob um protocolo congelado. Ela mediu o que os mecanismos entregam no primeiro dia de um estranho: agentes em uma árvore com harness leram menos bytes do que o controle sem harness nas duas tarefas, e as sessões de recurso desta versão consumiram menos entrada e saída de modelo do que as da versão maior anterior, conforme o harness reportou, em uma única carga de trabalho. A mesma avaliação marca os limites com clareza. O onboarding é um custo único que só se paga quando os fluxos são usados, e em cinco execuções novas nenhum agente entrou nos fluxos sozinho — os fluxos são comandos que você, ou um agente que sabe chamá-los, invoca. O segundo recurso na mesma árvore custou menos em todas as configurações medidas, incluindo o controle sem harness: parte dessa amortização pertence à própria árvore, e o harness existe para torná-la deliberada.
...
Como cada elemento é um arquivo no repositório, e não um recurso de uma ferramenta, o harness é portátil por construção: qualquer agente que leia markdown e execute comandos de shell pode abrir os mesmos arquivos, sem nenhum framework sob medida por ferramenta. Essa é a afirmação de uma linha sobre a qual o restante da metodologia se apoia: *o próprio repositório se torna o harness*. É uma afirmação sobre o que o repositório carrega, não uma garantia de que um modelo encontre de forma confiável o fluxo correto sem ajuda diante de uma solicitação nova — por isso o kit também inclui pontos de entrada explícitos `dwp-*` e um `SKILL.md` roteador: portabilidade de arquivos e roteamento confiável são propriedades diferentes, e apenas a primeira é construída por si só.
