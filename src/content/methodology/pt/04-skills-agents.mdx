...
A sub-skill author é deliberada quanto à adequação. Ela cria uma skill para um procedimento repetível que as pessoas executam manualmente, um agent para um papel recorrente com necessidades distintas de modelo ou ferramenta, e um command apenas como um ponto de entrada enxuto que encaminha para uma skill ou agent. Ela ignora qualquer coisa genérica que não corresponda a um fluxo de trabalho real e mantém o catálogo `.agents/docs/` em sincronia com o que quer que adicione. A mesma sub-skill sustenta a reconciliação de skills do Final Review, que reconcilia o catálogo com o que está realmente em disco — e o `/dwp-verify` confirma essa reconciliação de forma objetiva, falhando se o catálogo e os arquivos divergirem.

## Addons de manutenção

Além da autoria, o DWP fornece **addons**. Quatro são opcionais — nunca exigidos para que um repositório seja AI-first; o quinto, a **revisão local do AI Diff Reviewer**, faz parte da linha de base desde o padrão 2.3.0 (instalada pelo onboarding, executada por cada Final Review), e apenas sua superfície de CI é opcional. O addon **dependency-upgrade** é o exemplo canônico: ele raciocina sobre o gerenciador de pacotes *real* do repositório (npm, pnpm ou yarn com `ncu`; pip, poetry ou uv; Cargo; Go modules; Bundler; Composer) em vez de presumir um, e então atualiza em pequenos lotes, executa o validation gate real do repositório após cada lote, reverte qualquer lote que falhe e resume sem comitar automaticamente. O addon **design-system** é o exemplo de interface: quando um repositório tem uma superfície de interface voltada ao usuário, ele gera um `DESIGN.md` (em `docs/DESIGN.md`, referenciado a partir do `AGENTS.md`) que captura as convenções reais do repositório por perfil detectado — **UI visual** (cores, tipografia, espaçamento, componentes, regras de acessibilidade), **saída de CLI** (estilos semânticos de terminal, painéis e spinners, degradação TTY/`NO_COLOR`) e **conversacional** (voz da mensagem, anatomia e renderização por plataforma) — para que qualquer agente produza saída de interface consistente e fiel à marca. Quando um design system é detectado, a oferta é obrigatória mas a instalação fica protegida por aceitação: o perfil visual é recomendado com força quando detectado, os perfis de CLI e conversacional são recomendados quando detectados e sempre perguntados; o addon nunca é oferecido para um repositório sem nenhuma superfície de interface. Os addons opcionais são aceitos explicitamente durante o onboarding; recusar um deixa o repositório totalmente conforme.
...
A cadeia de ferramentas de teste e validação recebe atenção especial, porque os validation gates são o que torna o resultado de um plano confiável. Quando um repositório já tem testes e linters, o onboarding registra seus comandos e convenções reais. Quando não tem nenhum, o onboarding não se limita a anotar a lacuna — ele propõe uma configuração adequada à stack e a escreve no guia de testes do repositório, para que todo plano futuro tenha um gate real a executar em vez de um indefinido.
