...
Deep Work Plan ist die Disziplin, diese vorgelagerte Arbeit in einer dauerhaften, wiederholbaren Form zu leisten. Seine zwei Gründungspfeiler sind die beiden Hälften guter Richtung: Eine **Spezifikation** legt fest, wie „korrekt“ aussieht, und ein **Harness** gibt dem Agenten den Kontext und die Werkzeuge, um es zu erreichen. Eine dritte Disziplin hält die Kombination in großem Maßstab bezahlbar: die **Token-Effizienz** — denn ein Harness, der funktioniert, ist einer, den der Agent über einen langen Horizont tragen kann. Gemeinsam verwandeln sie die rohe Fähigkeit eines Modells in verlässliches Engineering — über eine Aufgabe hinweg aufrechterhalten, die stundenlang läuft, und über Agenten hinweg bewahrt, die zwischen Sitzungen wechseln.
...
<RepoAsHarness lang="de" class="not-prose my-10" />

Weil jedes Element eine Datei im Repository ist statt ein Feature eines Werkzeugs, ist das Harness von Natur aus portabel: Jeder Agent, der Markdown liest und Shell-Befehle ausführt, kann dieselben Dateien öffnen — ohne maßgeschneidertes Framework je Werkzeug. Das ist die einzeilige Behauptung, auf der der Rest der Methodik ruht: *Das Repository selbst wird zum Harness*. Das ist eine Aussage darüber, was das Repository trägt, keine Garantie, dass ein Modell bei einer neuen Anfrage unbeaufsichtigt zuverlässig den richtigen Flow findet — weshalb das Kit zusätzlich explizite `dwp-*`-Einstiegspunkte und ein Router-`SKILL.md` mitliefert: Dateiportabilität und zuverlässiges Routing sind unterschiedliche Eigenschaften, und nur die erste entsteht allein durch Konstruktion.

## Effizient durch Konstruktion

Der erste Pfeiler macht die Arbeit korrekt; der zweite macht sie portabel. Der dritte hält sie **bezahlbar** — denn Kontext ist die knappste Ressource, über die ein KI-Coding-Agent verfügt, und eine Methodik, die ihn verschwendet, scheitert genau dann, wenn die Arbeit lang wird.

Drei Mechanismen leisten diese Arbeit:

- **Schrittweises Nachladen.** Der Agent liest einen wesentlichen Kern, und jeder Abschnitt eines Leitfadens oder der Spezifikation wird erst geladen, wenn der Auslöser des Ablaufs selbst auslöst. Der Agent trägt das Kapitel, das er braucht — nicht das ganze Buch.
- **Validierung nach berührter Oberfläche.** Jede Aufgabe zeichnet auf, was sie ändert, und wählt ihr Gate nach Risikoklasse — eine kleine, isolierte Änderung wird eng nachgewiesen; eine Änderung an geteilter Mechanik wird breit nachgewiesen. Validierung wird nach Wirkung gewählt, nicht nach Gewohnheit.
- **Aufgabenlokales Lernen.** Wenn eine Aufgabe ein wiederverwendbares Muster hervorbringt, fällt die Entscheidung innerhalb dieser Aufgabe, wo die Belege frisch sind — nicht in einer Retrospektive, die niemand liest.

Nirgends steht die Behauptung, all das sei von Grund auf verlustfrei, und nirgends wird mit erfundenen Prozentwerten geworben. Die Instruction-Bytes, die jeder Ablauf benötigt, werden gemessen und veröffentlicht; die Aussagen zum Verhalten sind durch Replays mit gezielt eingeschleusten Fehlern belegt; und eine Evaluation mit frischen Agenten — dieselben zwei Features gebaut ohne Harness, mit der vorherigen Major-Version und mit dieser — ist unter einem eingefrorenen Protokoll bereits gelaufen. Sie maß, was die Mechanismen am ersten Tag eines Fremden liefern: Agenten auf einem Harness-tragenden Baum lasen in beiden Aufgaben weniger Bytes als die Harness-lose Kontrolle, und die Feature-Sitzungen dieser Version verbrauchten weniger Modell-Ein- und -Ausgabe als die der vorherigen Major-Version, wie der Harness sie berichtete, auf einer einzelnen Workload. Dieselbe Evaluation markiert die Grenzen deutlich. Onboarding ist ein einmaliger Aufwand, der sich nur zurückzahlt, wenn die Flows genutzt werden, und in fünf frischen Läufen betrat kein Agent die Flows von selbst — die Flows sind Befehle, die Sie oder ein Agent, der sie kennt, aufrufen. Das zweite Feature auf demselben Baum kostete in jeder gemessenen Konfiguration weniger, die Harness-lose Kontrolle eingeschlossen: Ein Teil dieser Amortisation gehört dem Baum selbst, und das Harness existiert, um sie bewusst zu machen.
