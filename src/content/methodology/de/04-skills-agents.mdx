...
<SkillsAgents lang="de" class="not-prose my-10" />
...
Die author-Sub-Skill achtet bewusst auf Passung. Sie erstellt eine Skill für eine wiederholbare Prozedur, die Menschen von Hand ausführen, einen Agenten für eine wiederkehrende Rolle mit eigenen Modell- oder Werkzeuganforderungen und einen Befehl nur als schlanken Einstiegspunkt, der an eine Skill oder einen Agenten weiterleitet. Sie überspringt alles Generische, das keinem echten Workflow entspricht, und hält den `.agents/docs/`-Katalog mit allem synchron, was sie hinzufügt. Dieselbe Sub-Skill stützt die Skills-Abstimmung des Final Review, die den Katalog mit dem abgleicht, was tatsächlich auf der Festplatte liegt — und `/dwp-verify` bestätigt diesen Abgleich objektiv und schlägt fehl, wenn Katalog und Dateien auseinandergehen.

## Wartungs-Add-ons

Über das Verfassen hinaus liefert DWP **Add-ons**. Vier sind Opt-in — nie erforderlich, damit ein Repository AI-first ist; das fünfte, die **lokale Überprüfung des AI Diff Reviewer**, ist seit Standard 2.3.0 Teil der Baseline (vom Onboarding installiert, von jedem Final Review ausgeführt), mit nur optionaler CI-Oberfläche. Das **dependency-upgrade**-Add-on ist das kanonische Beispiel: Es schlussfolgert über den *tatsächlichen* Paketmanager des Repositorys (npm, pnpm oder yarn mit `ncu`; pip, poetry oder uv; Cargo; Go modules; Bundler; Composer), statt einen anzunehmen, führt dann Upgrades in kleinen Chargen durch, lässt nach jeder Charge das echte Validierungs-Gate des Repositorys laufen, macht jede fehlgeschlagene Charge rückgängig und fasst zusammen, ohne automatisch zu committen. Das **design-system**-Add-on ist das Interface-Beispiel: Wenn ein Repository eine nutzerseitige Interface-Oberfläche hat, erzeugt es eine `DESIGN.md` (unter `docs/DESIGN.md`, von `AGENTS.md` referenziert), die die echten Konventionen des Repositorys je erkanntem Profil erfasst — **visuelle UI** (Farben, Typografie, Abstände, Komponenten, Barrierefreiheitsregeln), **CLI-Ausgabe** (semantische Terminal-Stile, Panels und Spinner, TTY-/`NO_COLOR`-Degradation) und **konversational** (Nachrichtenstimme, Nachrichtenanatomie und Rendering pro Plattform) —, damit jeder Agent markenkonforme, konsistente Interface-Ausgaben erzeugt. Wenn ein Designsystem erkannt wird, ist das Angebot verpflichtend, die Installation aber zustimmungsgesteuert: das visuelle Profil ist bei Erkennung dringend empfohlen, die CLI- und konversationalen Profile werden bei Erkennung empfohlen und stets erfragt; das Add-on wird nie für ein Repository ohne jede Interface-Oberfläche angeboten. Optionale Add-ons werden während des Onboardings explizit angenommen; lehnt man eines ab, bleibt das Repository vollständig konform.
