---
title: The core loop
description: "The create, execute, refine, resume, status, and verify workflow, the gitignored .dwp output directory, the ten-section task anatomy, and how validation works."
order: 2
lang: en
...
- **create** — Generate a new plan from a goal. The agent analyzes the goal, decomposes it into sequential tasks, and writes the plan files. It should ask clarifying questions before writing when the goal is ambiguous. In guided mode the plan is staged as a refined draft for review; in trust mode it materializes directly — the analysis and quality checks run either way.
...
Every task file contains these ten sections, in order. The structure guarantees that each unit of work is self-contained and reviewable:

1. **Goal** — one paragraph stating what the task achieves.
2. **Context** — background, links, and why this task exists.
3. **Touched Surface** — what the task changes, and the risk class that selects its validation gate.
4. **Steps** — ordered, concrete actions to perform.
5. **Acceptance criteria** — a checklist of conditions that define done.
6. **Validation** — the gate selected from the Touched Surface.
...
Between **Steps** and **Acceptance criteria** sits the section that makes validation deliberate: the **Touched Surface**. It records what the task plans to change, what it actually changed, the consumers that depend on it, and a risk class — *isolated*, *seam*, *shared/core*, or *unknown*. The class selects the gate: an isolated change proves itself narrowly; a change to shared machinery proves itself broadly.
