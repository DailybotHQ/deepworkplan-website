...
Deep Work Plan is the discipline of doing that upstream work in a durable, repeatable form. Its two founding pillars are the two halves of good direction: a **specification** states what "correct" looks like, and a **harness** gives the agent the context and tools to reach it. A third discipline keeps the combination affordable at scale: **token efficiency**, because a harness that works is one the agent can actually carry through a long horizon. Together they convert a model's raw ability into dependable engineering — sustained across a task that runs for hours, and preserved across agents that change between sessions.
...
Because every element is a file in the repository rather than a feature of a tool, the harness is portable by construction: any agent that reads markdown and runs shell commands can open the same files, with no bespoke per-tool framework. That is the one-line claim the rest of the methodology rests on: *the repository itself becomes the harness*. It is a claim about what the repository carries, not a guarantee that a model reliably finds the right flow unaided on a fresh request — which is why the kit also ships explicit `dwp-*` entry points and a router `SKILL.md`: file portability and reliable routing are different properties, and only the first is built by construction alone.

## Efficient by construction

The first pillar makes the work correct; the second makes it portable. The third keeps it **affordable** — because context is the scarcest resource a coding agent has, and a methodology that wastes it fails exactly when the work gets long.

Three mechanisms do the work:

- **Progressive disclosure.** The agent reads an essential core, and each guide or specification section loads only when the flow's own trigger fires. The agent carries the chapter it needs, not the whole book.
- **Validation selected by touched surface.** Every task records what it changes and picks its gate by risk class — a small, isolated change proves itself narrowly; a change to shared machinery proves itself broadly. Validation is chosen by effect, not by habit.
- **Task-local learning.** When a task produces a reusable pattern, the decision happens inside that task, where the evidence is fresh — not in a retrospective nobody reads.

None of this is claimed to be loss-free by construction, and none of it is marketed with invented percentages. The instruction bytes each flow requires are measured and published; the behavioral claims are backed by seeded-fault replays; and a fresh-agent evaluation — the same two features built with no harness, with the previous major version, and with this one — has now run under a frozen protocol. It measured what the mechanisms deliver on a stranger's first day: agents on a harness-bearing tree read fewer bytes than the no-harness control in both tasks, and this version's feature sessions consumed less model input and output than the previous major's, as the harness reported them, on a single workload. The same evaluation marks the limits plainly. Onboarding is a one-time cost that pays back only when the flows are used, and in five fresh runs no agent entered the flows unaided — the flows are commands, invoked by you or by an agent that knows to call them. The second feature on the same tree cost less in every configuration measured, the no-harness control included: part of that amortization belongs to the tree itself, and the harness exists to make it deliberate.
