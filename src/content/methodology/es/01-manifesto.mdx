---
title: Manifiesto
description: "Qué es un Deep Work Plan, por qué importa la ejecución autónoma estructurada y cómo convierte cualquier repo en código AI-first pilotable por agentes."
order: 1
lang: es
...
Deep Work Plan es la disciplina de hacer ese trabajo aguas arriba de forma duradera y repetible. Sus dos pilares fundacionales son las dos mitades de una buena dirección: una **especificación** declara qué aspecto tiene lo "correcto", y un **harness** le da al agente el contexto y las herramientas para alcanzarlo. Juntos convierten la capacidad bruta de un modelo en ingeniería confiable — sostenida a lo largo de una tarea que dura horas y preservada entre agentes que cambian de una sesión a otra. Una tercera disciplina mantiene la combinación asequible a escala: la **eficiencia de tokens**, porque un harness que funciona es uno que el agente puede cargar a través de un horizonte largo.

## Eficiente por construcción

El primer pilar hace el trabajo correcto; el segundo lo hace portable. El tercero lo mantiene **asequible** — porque el contexto es el recurso más escaso que tiene un agente de código, y una metodología que lo desperdicia falla justo cuando el trabajo se alarga.

Tres mecanismos hacen el trabajo:

- **Divulgación progresiva.** El agente lee un núcleo esencial, y cada guía o sección de la especificación se carga solo cuando el propio flujo dispara su desencadenante. El agente lleva el capítulo que necesita, no el libro entero.
- **Validación seleccionada por la superficie tocada.** Cada tarea registra lo que cambia y elige su puerta por clase de riesgo — un cambio pequeño y aislado se prueba de forma acotada; un cambio en maquinaria compartida se prueba a fondo.
- **Aprendizaje local por tarea.** Cuando una tarea produce un patrón reutilizable, la decisión ocurre dentro de esa tarea, donde la evidencia está fresca — no en una retrospectiva que nadie lee.

Nada de esto afirma ser sin pérdidas por construcción, ni se promociona con porcentajes inventados. Los bytes de instrucción que cada flujo requiere se miden y publican; las afirmaciones de comportamiento están respaldadas por replays con fallos sembrados; y una evaluación con agentes frescos — los mismos dos features construidos sin harness, con la versión mayor anterior y con esta — ya se ejecutó bajo un protocolo congelado. Midió lo que los mecanismos entregan el primer día de un desconocido: los agentes sobre un árbol con harness leyeron menos bytes que el control sin harness en ambas tareas, y las sesiones de feature de esta versión consumieron menos entrada y salida de modelo que las de la versión mayor anterior, tal como las reportó el harness, en una sola carga de trabajo. La misma evaluación marca los límites con claridad. El onboarding es un costo de una sola vez que solo se recupera cuando los flujos se usan, y en cinco ejecuciones frescas ningún agente entró a los flujos por sí solo — los flujos son comandos que invocas tú o un agente que sabe llamarlos. El segundo feature sobre el mismo árbol costó menos en todas las configuraciones medidas, incluido el control sin harness: parte de esa amortización le pertenece al árbol mismo, y el harness existe para hacerla deliberada.
...
<RepoAsHarness lang="es" class="not-prose my-10" />

Como cada elemento es un archivo del repositorio y no una función de una herramienta, el harness es portable por construcción: cualquier agente que lea Markdown y ejecute comandos de shell puede abrir los mismos archivos, sin un framework a medida por herramienta. Esa es la afirmación de una línea sobre la que descansa el resto de la metodología: *el propio repositorio se convierte en el harness*. Es una afirmación sobre lo que lleva el repositorio, no una garantía de que un modelo encuentre de forma fiable el flujo correcto sin ayuda ante una solicitud nueva — por eso el kit también incluye puntos de entrada explícitos `dwp-*` y un `SKILL.md` enrutador: la portabilidad de archivos y el enrutamiento fiable son propiedades distintas, y solo la primera se construye por sí sola.
