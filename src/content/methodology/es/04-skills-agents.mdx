...
La subhabilidad author es deliberada respecto al encaje. Crea una habilidad para un procedimiento repetible que las personas ejecutan a mano, un agente para un rol recurrente con necesidades distintas de modelo o herramientas, y un comando solo como punto de entrada ligero que enruta hacia una habilidad o un agente. Omite cualquier cosa genérica que no corresponda a un flujo real y mantiene el catálogo de `.agents/docs/` sincronizado con lo que añade. Esta misma subhabilidad respalda la reconciliación de skills del Final Review, que reconcilia el catálogo con lo que realmente hay en disco — y `/dwp-verify` confirma esa reconciliación de forma objetiva, fallando si el catálogo y los archivos divergen.

## Complementos de mantenimiento

Más allá de la autoría, DWP incluye **complementos**. Cuatro son opcionales — nunca obligatorios para que un repositorio sea AI-first; el quinto, la **revisión local de AI Diff Reviewer**, es parte de la línea base desde el estándar 2.3.0 (instalada por el onboarding, ejecutada por cada Final Review), y solo su superficie de CI es opcional. El complemento **dependency-upgrade** (actualización de dependencias) es el ejemplo canónico: razona sobre el gestor de paquetes *real* del repositorio (npm, pnpm o yarn con `ncu`; pip, poetry o uv; Cargo; módulos de Go; Bundler; Composer) en lugar de asumir uno, y luego actualiza en lotes pequeños, ejecuta el control de validación real del repositorio después de cada lote, revierte cualquier lote que falle y resume sin confirmar automáticamente. El complemento **design-system** (sistema de diseño) es el ejemplo de interfaz: cuando un repositorio tiene una superficie de interfaz orientada al usuario, genera un `DESIGN.md` (en `docs/DESIGN.md`, referenciado desde `AGENTS.md`) que captura las convenciones reales del repositorio por perfil detectado — **UI visual** (colores, tipografía, espaciado, componentes, reglas de accesibilidad), **salida de CLI** (estilos semánticos de terminal, paneles y spinners, degradación TTY/`NO_COLOR`) y **conversacional** (voz del mensaje, anatomía y renderizado por plataforma) — para que cualquier agente produzca salida de interfaz coherente y fiel a la marca. Cuando se detecta un sistema de diseño, la oferta es obligatoria pero la instalación está protegida por aceptación: el perfil visual es recomendado con fuerza cuando se detecta, los perfiles de CLI y conversacional se recomiendan cuando se detectan y siempre se preguntan; el complemento nunca se ofrece para un repositorio sin ninguna superficie de interfaz. Los complementos opcionales se aceptan de forma explícita durante la incorporación; rechazar uno deja el repositorio plenamente conforme.
...
La cadena de herramientas de pruebas y validación recibe atención especial, porque las puertas de validación son lo que hace fiable el resultado de un plan. Cuando un repositorio ya tiene pruebas y linters, la incorporación registra sus comandos y convenciones reales. Cuando no tiene ninguno, la incorporación no se limita a señalar la carencia: propone una configuración adecuada al stack y la escribe en la guía de pruebas del repositorio, de modo que todo plan futuro tenga una puerta real que ejecutar en lugar de una indefinida.
