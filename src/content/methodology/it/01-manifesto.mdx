...
Deep Work Plan è la disciplina di svolgere quel lavoro a monte in una forma duratura e ripetibile. I suoi due pilastri fondativi sono le due metà di una buona direzione: una **specifica** definisce cosa significhi «corretto» e una **harness** dà all’agente il contesto e gli strumenti per raggiungerlo. Insieme convertono la capacità grezza di un modello in ingegneria affidabile — sostenuta lungo un’attività che dura ore e preservata tra agenti che cambiano da una sessione all’altra. Una terza disciplina mantiene la combinazione sostenibile su scala: l’**efficienza dei token**, perché un harness che funziona è uno che l’agente può portare attraverso un orizzonte lungo.

## Efficiente per costruzione

Il primo pilastro rende il lavoro corretto; il secondo lo rende portatile. Il terzo lo mantiene **sostenibile** — perché il contesto è la risorsa più scarsa che un agente di coding abbia, e una metodologia che lo spreca fallisce esattamente quando il lavoro si allunga.

Tre meccanismi fanno il lavoro:

- **Divulgazione progressiva.** L’agente legge un nucleo essenziale, e ogni guida o sezione della specifica si carica solo quando il flusso stesso attiva il suo trigger. L’agente porta con sé il capitolo di cui ha bisogno, non l’intero libro.
- **Validazione selezionata dalla superficie toccata.** Ogni attività registra ciò che modifica e sceglie il suo varco per classe di rischio — una modifica piccola e isolata si prova in modo circoscritto; una modifica a machineria condivisa si prova ampiamente.
- **Apprendimento locale per attività.** Quando un’attività produce un pattern riutilizzabile, la decisione avviene dentro di essa, dove l’evidenza è fresca — non in una retrospettiva che nessuno legge.

Nulla di questo dichiara di essere senza perdite per costruzione, né viene promosso con percentuali inventate. I byte di istruzione che ogni flusso richiede sono misurati e pubblicati; le affermazioni comportamentali sono supportate da replay con errori seminati; e una valutazione con agenti freschi — le stesse due funzionalità costruite senza harness, con la versione maggiore precedente e con questa — è ormai stata eseguita sotto un protocollo congelato. Ha misurato ciò che i meccanismi consegnano il primo giorno di uno sconosciuto: gli agenti su un albero con harness leggevano meno byte del controllo senza harness in entrambi i task, e le sessioni di funzionalità di questa versione consumavano meno input e output di modello di quelle della versione maggiore precedente, come riportato dall’harness, su un singolo carico di lavoro. La stessa valutazione segna i limiti con chiarezza. L’onboarding è un costo una tantum che si ripaga solo quando i flussi vengono usati, e in cinque esecuzioni fresche nessun agente è entrato nei flussi da solo — i flussi sono comandi che invocate voi o un agente che sa chiamarli. La seconda funzionalità sullo stesso albero è costata meno in ogni configurazione misurata, controllo senza harness incluso: parte di quell’ammortamento appartiene all’albero stesso, e l’harness esiste per renderlo deliberato.
...
Poiché ogni elemento è un file nel repository anziché una funzionalità di uno strumento, la harness è portabile per costruzione: qualsiasi agente che legge Markdown ed esegue comandi di shell può aprire gli stessi file, senza alcun framework su misura per strumento. È questa l’affermazione di una riga su cui poggia il resto della metodologia: *il repository stesso diventa la harness*. È un’affermazione su ciò che il repository porta con sé, non una garanzia che un modello trovi in modo affidabile il flusso giusto senza aiuto su una richiesta nuova — per questo il kit fornisce anche punti di ingresso `dwp-*` espliciti e uno `SKILL.md` router: la portabilità dei file e l’instradamento affidabile sono proprietà diverse, e solo la prima è costruita per costruzione da sola.
