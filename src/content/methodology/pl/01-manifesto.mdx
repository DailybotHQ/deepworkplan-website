...
Deep Work Plan to dyscyplina wykonywania tej wstępnej pracy w trwałej, powtarzalnej formie. Jego dwa filary założycielskie to dwie połowy dobrego kierunku: **specyfikacja** określa, jak wygląda „poprawne”, a **harness** daje agentowi kontekst i narzędzia, by to osiągnąć. Trzecia dyscyplina sprawia, że ta kombinacja pozostaje przystępna kosztowo w skali: **efektywność tokenów** — bo harness, który działa, to taki, który agent jest w stanie nieść przez długi dystans. Razem zamieniają surową zdolność modelu w niezawodną inżynierię — podtrzymywaną przez zadanie trwające godzinami i zachowaną pomiędzy agentami, którzy zmieniają się między sesjami.
...
Ponieważ każdy element jest plikiem w repozytorium, a nie funkcją narzędzia, harness jest przenośny z samej konstrukcji: dowolny agent, który czyta markdown i uruchamia polecenia powłoki, może otworzyć te same pliki, bez autorskiego frameworka per narzędzie. To jednolinijkowa teza, na której opiera się reszta metodyki: *samo repozytorium staje się harness*. Jest to twierdzenie o tym, co niesie repozytorium, a nie gwarancja, że model niezawodnie znajdzie właściwy przepływ bez pomocy przy świeżym żądaniu — dlatego kit dostarcza również jawne punkty wejścia `dwp-*` oraz router `SKILL.md`: przenośność plików i niezawodne routowanie to różne właściwości, a tylko ta pierwsza jest zbudowana z samej konstrukcji.

## Wydajność z samej konstrukcji

Pierwszy filar czyni pracę poprawną; drugi czyni ją przenośną. Trzeci dba o to, by pozostawała **przystępna kosztowo** — bo kontekst to najrzadszy zasób, jakim dysponuje agent kodujący, a metodyka, która go marnuje, zawodzi dokładnie wtedy, gdy praca robi się długa.

Pracę tę wykonują trzy mechanizmy:

- **Stopniowe odsłanianie.** Agent czyta istotny rdzeń, a każdy rozdział przewodnika czy specyfikacji ładuje się dopiero, gdy zadziała własny wyzwalacz danego przepływu. Agent niesie rozdział, którego potrzebuje — nie całą książkę.
- **Walidacja dobrana według powierzchni dotkniętej.** Każde zadanie zapisuje, co zmienia, i wybiera swoją bramkę według klasy ryzyka — mała, izolowana zmiana wymaga wąskiego dowodu, a zmiana we wspólnej mechanice dowodu szerokiego. O walidacji decyduje skutek, nie nawyk.
- **Uczenie wewnątrz zadania.** Kiedy zadanie wytwarza wzorzec nadający się do ponownego użycia, decyzja zapada wewnątrz tego zadania, gdzie dowody są świeże — a nie podczas retrospektywy, której nikt nie czyta.

Nic z tego nie jest obiecane jako wolne od strat z samej konstrukcji i nic z tego nie jest reklamowane wymyślonymi procentami. Liczba bajtów instrukcji, których wymaga każdy przepływ, jest mierzona i publikowana; twierdzenia o zachowaniu opierają się na odtworzonych przebiegach z podłożonymi błędami; a ewaluacja na świeżych agentach — te same dwa featury zbudowane bez harnessu, z poprzednią wersją główną i z obecną — została już przeprowadzona według zamrożonego protokołu. Zmierzono w niej, co mechanizmy dają w pierwszy dzień przybysza: agenci na drzewie z harnessem czytali mniej bajtów niż kontrola bez harnessu w obu zadaniach, a sesje featurów tej wersji zużywały mniej wejścia i wyjścia modelu niż poprzednia wersja główna — tak, jak zaraportował harness, na pojedynczym obciążeniu. Ta sama ewaluacja wyraźnie zaznacza granice. Onboarding to jednorazowy koszt, który zwraca się tylko przy używaniu przepływów, a w pięciu świeżych przebiegach żaden agent nie wszedł w przepływy sam — przepływy to komendy, które wywołujesz Ty albo agent, który wie, jak je wywołać. Drugi featur na tym samym drzewie kosztował mniej w każdej zmierzonej konfiguracji, łącznie z kontrolą bez harnessu: część tej amortyzacji należy do samego drzewa, a harness istnieje po to, by czynić ją świadomą.
