...
Deep Work Plan est la discipline qui consiste à accomplir ce travail en amont sous une forme durable et reproductible. Ses deux piliers fondateurs sont les deux moitiés d’une bonne direction : une **spécification** énonce à quoi ressemble le « correct », et un **harness** donne à l’agent le contexte et les outils pour l’atteindre. Ensemble, ils convertissent la capacité brute d’un modèle en ingénierie fiable — soutenue tout au long d’une tâche qui dure des heures, et préservée entre des agents qui changent d’une session à l’autre. Une troisième discipline maintient l’ensemble abordable à l’échelle : l’**efficacité des tokens**, car un harness qui fonctionne est un harness que l’agent peut porter sur un long horizon.

## Efficace par construction

Le premier pilier rend le travail correct ; le second le rend portable. Le troisième le maintient **abordable** — parce que le contexte est la ressource la plus rare qu’un agent de codage possède, et une méthodologie qui le gaspille échoue précisément quand le travail s’allonge.

Trois mécanismes font le travail :

- **Divulgation progressive.** L’agent lit un noyau essentiel, et chaque guide ou section de la spécification se charge uniquement lorsque le déclencheur du flux se déclenche. L’agent porte le chapitre dont il a besoin, pas le livre entier.
- **Validation sélectionnée par la surface touchée.** Chaque tâche enregistre ce qu’elle modifie et choisit sa porte par classe de risque — un changement petit et isolé se prouve de manière délimitée ; un changement de machinerie partagée se prouve largement.
- **Apprentissage local par tâche.** Lorsqu’une tâche produit un modèle réutilisable, la décision se prend en son sein, où la preuve est fraîche — pas dans une rétrospective que personne ne lit.

Rien de tout cela ne prétend être sans perte par construction, ni n’est promu avec des pourcentages inventés. Les octets d’instructions que chaque flux exige sont mesurés et publiés ; les affirmations comportementales sont appuyées par des relectures avec fautes semées ; et une évaluation à agents frais — les deux mêmes fonctionnalités construites sans harness, avec la version majeure précédente et avec celle-ci — s’est déjà déroulée sous un protocole figé. Elle a mesuré ce que les mécanismes livrent le premier jour d’un inconnu : les agents sur un arbre porteur de harness lisaient moins d’octets que le contrôle sans harness dans les deux tâches, et les sessions de fonctionnalité de cette version consommaient moins d’entrée et de sortie de modèle que celles de la version majeure précédente, telles que le harness les a rapportées, sur une seule charge de travail. La même évaluation marque les limites sans détour. L’onboarding est un coût unique qui ne se rembourse que si les flux sont utilisés, et sur cinq exécutions fraîches aucun agent n’est entré dans les flux de lui-même — les flux sont des commandes que vous, ou un agent qui sait les appeler, invoquez. La deuxième fonctionnalité sur le même arbre a coûté moins cher dans toutes les configurations mesurées, contrôle sans harness inclus : une part de cet amortissement appartient à l’arbre lui-même, et le harness existe pour la rendre délibérée.
...
Parce que chaque élément est un fichier du dépôt plutôt qu’une fonctionnalité d’un outil, le harness est portable par construction : tout agent qui lit du markdown et exécute des commandes shell peut ouvrir les mêmes fichiers, sans aucun framework sur mesure propre à un outil. C’est l’affirmation en une ligne sur laquelle repose le reste de la méthodologie : *le dépôt lui-même devient le harness*. C’est une affirmation sur ce que le dépôt porte, pas une garantie qu’un modèle trouve de manière fiable le bon flux sans aide sur une demande inédite — c’est pourquoi le kit fournit aussi des points d’entrée `dwp-*` explicites et un `SKILL.md` routeur : la portabilité des fichiers et un routage fiable sont des propriétés différentes, et seule la première est construite par construction seule.
