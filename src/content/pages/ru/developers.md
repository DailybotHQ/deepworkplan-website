...
## Планируйте и выполняйте с помощью skill

API выше позволяет агенту читать этот сайт. Skill DWP — это то, что позволяет агенту выполнять методологию: установите её один раз в репозитории, и она принесёт роутер плюс девять sub-skills, вызываемых как слэш-команды.

- **Lite** — записи задач прямо в README плана, для небольшой, ограниченной работы.
- **Full** — один файл на задачу, для работы на долгую перспективу или с реальными зависимостями между задачами.
- **Guided (по умолчанию)** — агент материализует план, готовый к ревью, и спрашивает перед началом продуктовой работы.
- **Trust (или auto)** — пропускает раунд ревью и сразу выполняет заранее одобренный план.

| Команда | Описание |
|---------|----------|
| `/dwp-create <goal>` | Превращает цель в план — Lite по умолчанию, Full для более крупной работы. |
| `/dwp-execute` | Выполняет существующий план задача за задачей: читает его полностью, выполняет каждую задачу по порядку, валидирует её gate, обновляет прогресс. |
| `/dwp-refine` | Добавляет, удаляет или переупорядочивает задачи в существующем плане, сохраняя завершённую работу и её зафиксированные свидетельства. |
| `/dwp-resume` | Восстанавливает состояние из собственных файлов плана и продолжает прерванный план с первой незавершённой задачи. |
| `/dwp-status` | Сообщает о прогрессе плана — завершённые, выполняемые, ожидающие — не внося никаких изменений. |
| `/dwp-verify` | Механически проверяет, является ли репозиторий AI-first и хорошо ли сформированы его планы. Ничего не меняет; сообщает pass или fail. |
| `/deepworkplan-onboard` | Делает репозиторий AI-first: анализирует его стек, затем генерирует адаптированный AGENTS.md, docs/, .agents/ и игнорируемый git .dwp/. |
| `/skill-create`, `/agent-create` | Sub-skill для авторства: развивает собственный кит репозитория — skill или агента. |
| `/dwp-upgrade` | Проверяет наличие более нового релиза skill и, только после одобрения, устанавливает его и заново запускает onboarding. |

Маленькое, ограниченное исправление — Lite, trust:
...
Работа на долгую перспективу — Full, guided:
...
Вывод каждого плана — манифест, лог прогресса, записи задач, свидетельства gate — живёт под игнорируемой git директорией `.dwp/` в самом репозитории. Ничего не отправляется в deepworkplan.com и не хранится им; skill вообще не делает сетевых вызовов.
