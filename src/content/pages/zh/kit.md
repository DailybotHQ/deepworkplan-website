...
- **AI Diff Reviewer** —— 通过结构化的本地评审（[AI Diff Reviewer](https://github.com/DailybotHQ/ai-diff-reviewer) v2 + 必需的 `.review/extension.md`）增强安全审查；可选的 Flow B 添加一个共享同一扩展的 CI PR 合并门控。始终询问 Flow A 还是 Flow B；绝不自动安装。
