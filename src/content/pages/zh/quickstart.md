...
代理与命令。接入流程会安装必备的 AI Diff Reviewer 本地审查（其 CI 门控保持可选），并提供四个可选附加组件——devcontainer、Dailybot、
dependency-upgrade 与 design-system——你只在它们契合时才采纳。一个仓库即便不带任何可选附加组件，也完全符合规范。
