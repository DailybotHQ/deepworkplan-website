...
工作。选择关乎表示成本，而非严谨程度：两种格式都携带稳定的任务 id、触及面、
验收标准、验证关卡和完成证据，因此 Lite 计划是一份真正的计划，而不是草稿。
格式、具体化、批准和执行是相互独立的维度；当紧凑记录不再足以承载某项要求
或关卡时，Lite 计划会通过 `/dwp-refine promote` 升级为 Full。
...
失败、仓库出现偏离，或需要新的批准或凭据，代理会记录阻塞并停止。任何流程
都不会为了宣称完成而削弱验证关卡。
