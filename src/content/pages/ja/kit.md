...
- **Design system** — フロントエンドに限定された `DESIGN.md`（`docs/DESIGN.md` に置かれ、`AGENTS.md` から参照される）で、リポジトリの実際のデザイントークンから推論され、エージェントがオンブランドの UI を生成できるようにします。デザインシステムが検出されたときはデフォルトでオンです。
