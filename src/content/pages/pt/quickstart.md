...
pequeno e delimitado — ou **Full**, um arquivo por tarefa, para trabalho de
longo alcance. A escolha é sobre o custo de representação, não sobre o
rigor: ambos os formatos carregam ids de tarefa estáveis, uma Superfície
tocada, critérios de aceitação, gates de validação e evidências de
conclusão, então um plano Lite é um plano real, não um rascunho. Formato,
materialização, aprovação e execução são eixos independentes; um plano
Lite é promovido a Full com `/dwp-refine promote` quando os registros
compactos deixam de carregar um requisito ou um gate.
...
DWP declarado, autoridade limitada e condições de parada explícitas. Se um
gate falhar fora do escopo de reparo planejado, o repositório divergir, ou
uma nova aprovação ou credencial for necessária, o agente registra o
bloqueio e para. Nenhum fluxo enfraquece um gate de validação para
reivindicar conclusão.
...
(`npx --yes skills add DailybotHQ/ai-diff-reviewer@v2.3.1 --skill ai-diff-reviewer -y`) mais um
