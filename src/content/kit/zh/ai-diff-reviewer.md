...
在 v2.0.1 与 v2.3.0 之间上游发布了三个版本。它们都没有改变本附加组件接入审查器的方式——Flow A、三条检测路径和阻断契约均保持不变——但确实改变了采用者所获得的能力。
...
| **后续轮次审查真正的新增 diff**（v2.1.0、v2.2.0） | 未决结果会向后传递。`prior-findings-resolution` 默认为 `advisory`：模型给出的"已解决"裁定会被报告，但该结果仍持续阻断，直到维护者关闭该讨论串。 |
