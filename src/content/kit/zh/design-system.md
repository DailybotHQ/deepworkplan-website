...
- **`visual-ui` 在检测到时受到强烈推荐** —— 一个带有 CSS 自定义属性的样式表、一份 Tailwind 配置或 `@theme` 块、UI 组件，或一份品牌/样式指南。检测使提供该提案成为必须，但绝不使安装自动化——只有在明确接受之后才会应用该配置档，引导模式与信任模式一视同仁。
- **`cli-output` 与 `conversational` 在检测到时被推荐——并且始终先询问，绝不自动应用**，引导模式与信任模式一视同仁。一个 CLI 渲染库外加一个刻意构建的显示层标志着前者；一个聊天平台 SDK 或消息组装层标志着后者。一个只做原始打印的简单参数解析器并不符合条件。

它从不是必需的——一个不带任何可选附加组件的仓库完全符合规范，你也始终可以拒绝任意配置档或整个附加组件。一份在配置档存在之前创建的 `DESIGN.md` 是一份有效的单配置档可视化文件：无需迁移。
