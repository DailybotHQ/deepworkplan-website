...
核心 Deep Work Plan 方法论对 Dailybot **零**依赖。零可选附加组件的仓库完全符合规范。仅在开发者或团队已在使用 Dailybot 时提供此附加组件；切勿为所有人自动安装。
...
在 DWP `onboard` **第 7b 阶段**，核心 AI-first 脚手架完成后，流程提供四个可选附加组件（AI Diff Reviewer 本地审查已在第 7a 阶段安装）。若开发者接受 Dailybot：
