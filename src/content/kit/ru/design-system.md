...
- **`visual-ui` настоятельно рекомендуется при обнаружении** — таблица стилей с пользовательскими CSS-свойствами, конфигурация Tailwind или блок `@theme`, UI-компоненты либо руководство по бренду/стилю. Обнаружение делает предложение обязательным, но никогда не делает установку автоматической — профиль применяется только после явного согласия, как в направляемом режиме, так и в режиме доверия.
- **`cli-output` и `conversational` рекомендуются при обнаружении — и о них всегда спрашивают, они никогда не применяются автоматически**, как в направляемом режиме, так и в режиме доверия. О первом сигнализирует библиотека отрисовки CLI вместе с осознанным слоем отображения; о втором — SDK чат-платформы или слой композиции сообщений. Голый парсер аргументов с сырыми выводами не подходит.

Оно никогда не обязательно — репозиторий с нулём опциональных дополнений полностью соответствует стандарту, и вы всегда можете отказаться от любого профиля или от всего дополнения. `DESIGN.md`, созданный до появления профилей, — это валидный визуальный файл с одним профилем: никакой миграции.
