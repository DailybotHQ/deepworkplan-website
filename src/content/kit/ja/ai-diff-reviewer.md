...
`create` はすべての Final Review のセキュリティパスにローカルレビューステップを追加し、`execute` がそれを実行します。出力は `analysis_results/SECURITY_REVIEW.md` の `## AI Diff Reviewer local review` の下に追記されます。

- **レビュアーの欠落 — 記録され、黙ってスキップされない：** スキルまたは拡張の欠落は `local reviewer not installed` の検出結果になります。実行がハーネスに書き込める場合（trust モードまたは明示的な承認）は、エージェントが欠けている部品をインストールしてからレビューし、そうでなければ検出結果は完了レポートに持ち越されます。
