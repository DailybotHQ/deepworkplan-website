...
- **`visual-ui` は検出されたとき強く推奨** — CSS カスタムプロパティを備えたスタイルシート、Tailwind の設定や `@theme` ブロック、UI コンポーネント、あるいはブランド／スタイルガイド。検出によって提供が必須になりますが、インストールが自動化されることはありません — このプロファイルが適用されるのは、あなたが明示的に受け入れた後だけです。ガイドモードでもトラストモードでも同様です。
- **`cli-output` と `conversational` は検出されたとき推奨され、必ず尋ねられ、決して自動適用されません**。ガイドモードでもトラストモードでも同様です。CLI のレンダリングライブラリと意図的な表示レイヤーが前者の、チャットプラットフォームの SDK やメッセージ組み立てレイヤーが後者の合図です。生の print を伴うだけの素朴な引数パーサーは該当しません。

これが必須となることは決してなく、オプションアドオンがゼロのリポジトリも完全に適合しており、どのプロファイルもアドオン全体も、いつでも辞退できます。プロファイルが存在する前に作られた `DESIGN.md` は、有効な単一プロファイルのビジュアルファイルです。移行は不要です。
