...
- **`visual-ui` được khuyến nghị mạnh mẽ khi được phát hiện** — một stylesheet với các CSS custom property, một cấu hình Tailwind hay khối `@theme`, các component UI, hay một hướng dẫn thương hiệu/phong cách. Việc phát hiện khiến đề xuất trở thành bắt buộc nhưng không bao giờ khiến việc cài đặt tự động — profile chỉ được áp dụng sau khi bạn chấp nhận rõ ràng, như nhau ở cả chế độ có hướng dẫn lẫn chế độ tin cậy.
- **`cli-output` và `conversational` được khuyến nghị khi được phát hiện — và luôn được hỏi, không bao giờ tự động áp dụng**, như nhau ở cả chế độ có hướng dẫn lẫn chế độ tin cậy. Một thư viện render CLI cộng với một lớp hiển thị có chủ đích báo hiệu profile thứ nhất; một SDK nền tảng chat hay một lớp soạn thông điệp báo hiệu profile thứ hai. Một trình phân tích đối số trần với các lệnh in thô không đủ điều kiện.
