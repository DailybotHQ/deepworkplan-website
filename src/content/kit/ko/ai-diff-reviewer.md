...
Deep Work Plan 실행을 **[AI Diff Reviewer](https://github.com/DailybotHQ/ai-diff-reviewer)**(마켓플레이스 목록 **"AI Diff Reviewer"**, 현재 버전 **v2.3.0**)에 연결하여, 필수 **Final Review**의 보안 점검이 구조화된 로컬 검토 — 판정, 결과 표, 심각도 — 를 실행하도록 하고, Flow B를 선택하면 모든 풀 리퀘스트를 CI에서 동일한 검토로 게이트할 수 있습니다. 표준 2.3.0부터 **로컬 리뷰는 기준선의 일부**입니다. 온보딩이 이를 설치하고 모든 Final Review가 이를 실행합니다. 옵트인인 것은 CI 표면뿐입니다.
