...
- **Pre-approved plan.** The plan was approved before any unattended turn — in guided mode the human approved the refined draft; in trust mode the developer's explicit `trust` instruction is the recorded approval. An agent MUST NOT create and execute a plan unattended in one turn without either; plan approval is the human control point.
