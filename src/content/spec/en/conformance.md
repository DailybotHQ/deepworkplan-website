...
5. The plan MUST persist progress so that work survives interruption and can be resumed by a different agent — and it MUST close with its recorded final review. A plan authored under this version MUST end with exactly one mandatory **Final Review** — the security pass, the final-state validation, and skills reconciliation. A plan authored under an earlier version ends with the three mandatory final tasks (Security Review, Skills & Agents Discovery, Executive Report) and remains conformant. A critical security finding blocks completion until fixed or explicitly accepted.
