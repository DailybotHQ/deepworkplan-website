...
- **Mode terpandu (default).** Alur menghasilkan tepat **satu** artefak untuk ditinjau: sebuah **draf yang disempurnakan** di `.dwp/drafts/`, dengan struktur yang cukup untuk disetujui atau dimintakan perubahan dalam satu kali jalan. Folder rencana baru dimaterialisasikan setelah persetujuan.
- **Mode trust (`trust` / `auto`).** Alur **langsung melakukan materialisasi rencana**, tanpa berkas draf — pengembang telah melepaskan tinjauan perantara. Analisis kebutuhan, pengurutan dependensi, dan sebuah **pemeriksaan kualitas rencana** tetap berjalan: trust melepaskan *tinjauannya*, bukan *analisisnya*. Rencana mode trust dicatat sebagai **disetujui di muka untuk eksekusi tanpa pengawasan**.
- **Mode draf eksplisit.** `refined-draft {name}` dan `from-refined-draft {file}` tetap tersedia di kedua mode; pengembang yang meminta sebuah draf akan mendapatkannya.

Dalam setiap mode, alur ini tidak pernah menghasilkan draf perantara yang belum disempurnakan sebagai langkah tinjauan terpisah.
