...
안내형 create는 **검토 가능한 대기 중 제안**을 작성합니다 — Lite든 Full이든, 이미 진짜 계획이며 버려질 초안이 아닙니다. 신뢰 모드는 **준비 완료되고 사전 승인된** 계획을 곧바로 구체화하고 즉시 제어를 돌려줍니다. `create`와 승격은 제품 작업을 결코 실행하지 않습니다; `execute`는 승인을 기다리는 `pending` 제안을 거부해야(MUST) 하며, 미해결 승격이 진행 중인 계획도 거부해야(MUST) 합니다.
