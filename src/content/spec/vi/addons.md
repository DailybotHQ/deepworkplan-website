...
Addon **design-system** là một phần mở rộng tự nguyện, giới hạn ở frontend, trao cho một repository một `DESIGN.md` — một tệp hệ thống thiết kế dạng Markdown mà bất kỳ coding agent nào cũng đọc để sinh ra UI nhất quán với hệ thống thiết kế của riêng repository. Nó suy luận về các token thiết kế thực của repository (các CSS custom property, một cấu hình Tailwind, các tệp token, style của component) thay vì sao chép một tệp thương hiệu, và tài liệu hóa các phần chuẩn tắc: màu sắc và vai trò màu (sáng và tối), kiểu chữ, bố cục và khoảng cách, độ nâng, hình khối, component, hành vi responsive, nên và không nên (gồm cả các quy tắc khả truy cập của repository), và một hướng dẫn prompt cho agent. Nó kiểm tra độ tương phản WCAG AA và tính toàn vẹn của token, và hòa giải một `DESIGN.md` đã có thay vì đè bừa lên nó.

Tệp này nằm tại `docs/DESIGN.md`, đặt cạnh các đặc tả khác của repository, và được tham chiếu từ `AGENTS.md` để agent khám phá ra nó theo cùng cách chúng khám phá phần còn lại của tài liệu (gốc repository chỉ được dùng khi không có cây thư mục `docs/`). Khám phá là bằng tham chiếu, không phải bằng vị trí vật lý. Addon này **bật mặc định khi được phát hiện**: khi một bề mặt UI hiện diện, quá trình khởi tạo áp dụng nó ở chế độ tin cậy và mạnh mẽ khuyến nghị nó ở chế độ có hướng dẫn; nó không bao giờ được đề xuất cho một repository chỉ có backend, CLI, hay thư viện, và một repository với không có addon nào vẫn hoàn toàn tuân thủ.

Tệp hệ thống thiết kế cấp repository này khác với một tài liệu thiết kế kỹ thuật theo tính năng (`design.md` kiểu "requirements → design → tasks" của các quy trình spec-driven gắn với công cụ). DWP không đi kèm một archetype tài liệu thiết kế theo tính năng riêng: README của một kế hoạch, tiêu chí chấp nhận của mỗi tác vụ, và các cổng kiểm chứng đã bao quát vai trò đó. Addon lấp đúng một khoảng trống mà vai trò đó không lấp: ngữ cảnh thiết kế UI bền vững, gốc-repo.
