...
- **Geführter Modus (Standard).** Der Ablauf erzeugt genau **ein** Artefakt zur Review: einen **überarbeiteten Entwurf** in `.dwp/drafts/`, mit genug Struktur, um in einem Durchgang zu genehmigen oder Änderungen anzufordern. Der Plan-Ordner wird erst nach der Genehmigung materialisiert.
- **Trust-Modus (`trust` / `auto`).** Der Ablauf **materialisiert den Plan direkt**, ohne Entwurfsdatei — der Entwickler hat auf die zwischengeschaltete Review verzichtet. Die Anforderungsanalyse, die Abhängigkeitsreihenfolge und eine **Plan-Qualitätsprüfung** laufen dennoch: Trust verzichtet auf die *Review*, nicht auf die *Analyse*. Ein Trust-Modus-Plan wird als **für unbeaufsichtigte Ausführung vorgenehmigt** aufgezeichnet.
- **Explizite Entwurfsmodi.** `refined-draft {name}` und `from-refined-draft {file}` bleiben in beiden Modi verfügbar; ein Entwickler, der einen Entwurf anfordert, bekommt einen.

In jedem Modus erzeugt der Ablauf niemals einen zwischengeschalteten, nicht überarbeiteten Entwurf als separaten prüfbaren Schritt.
