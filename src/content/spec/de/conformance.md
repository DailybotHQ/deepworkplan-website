...
3. **Eine `docs/`-Hierarchie.** Das Repository MUSS ein `docs/`-Verzeichnis enthalten, das die Standardkategorien (Architektur, Standards, Testing, Entwicklungsbefehle, Sicherheit und Agenten-Onboarding) mit echtem, repository-spezifischem Inhalt abdeckt. Komplexe Module SOLLTEN ihre eigene `README.md` tragen.
...
2. Der Plan MUSS den Fortschritt persistieren, sodass die Arbeit eine Unterbrechung übersteht und von einem anderen Agenten wiederaufgenommen werden kann.
3. Der Plan MUSS die zwei verpflichtenden Abschlussaufgaben enthalten — Skills & Agents Discovery und den Executive Report.
4. Aufgaben SOLLTEN sich vor der Ausführung erneut am Ziel des Plans verankern, um ein Abdriften über einen langen Horizont zu verhindern.
