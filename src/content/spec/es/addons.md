...
El complemento **design-system** (sistema de diseño) es una extensión opcional con alcance de frontend que da a un repositorio un `DESIGN.md` — un archivo Markdown de sistema de diseño que cualquier agente de codificación lee para generar una UI coherente con el sistema de diseño propio del repositorio. Razona sobre los tokens de diseño reales del repositorio (propiedades personalizadas de CSS, una configuración de Tailwind, archivos de tokens, estilos de componentes) en lugar de copiar el archivo de una marca, y documenta las secciones canónicas: colores y roles (claro y oscuro), tipografía, maquetación y espaciado, elevación, formas, componentes, comportamiento responsive, recomendaciones y prohibiciones (incluidas las reglas de accesibilidad del repositorio) y una guía de prompts para agentes. Comprueba el contraste WCAG AA y la integridad de los tokens, y reconcilia un `DESIGN.md` existente en lugar de arrasarlo.

El archivo vive en `docs/DESIGN.md`, junto a las demás especificaciones del repositorio, y se referencia desde `AGENTS.md` para que los agentes lo descubran de la misma forma que descubren el resto de la documentación (la raíz del repositorio se usa solo cuando no existe un árbol `docs/`). El descubrimiento es por referencia, no por ubicación física. El complemento está **activado por defecto cuando se detecta**: cuando hay una superficie de UI presente, la incorporación lo aplica en modo de confianza y lo recomienda con fuerza en modo guiado; nunca se ofrece para un repositorio de backend, CLI o solo de biblioteca, y un repositorio con cero complementos sigue siendo plenamente conforme.
