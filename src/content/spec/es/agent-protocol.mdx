...
**Versión 1.3.** Este protocolo define cómo DEBE comportarse un agente de programación con IA cuando trabaja con Deep Work Plans. Las palabras clave MUST (DEBE), SHOULD (DEBERÍA) y MAY (PUEDE) siguen el RFC 2119.
...
- Escribir cada tarea con la anatomía de diez secciones — la Superficie tocada registra qué cambia la tarea y selecciona su puerta de validación por clase de riesgo.
- Terminar con el único Final Review obligatorio (pase de seguridad, validación de estado final, reconciliación de skills).
...
- Avanzar desde una puerta que pasa a la siguiente tarea sin pedir confirmación; intentar reparaciones dentro del alcance autorizado de la tarea cuando una puerta falla — con una hipótesis concreta, deteniendo los reintentos a ciegas tras dos intentos sin evidencia nueva.
- Detenerse y preguntar solo en un límite de aprobación genuino o ante un bloqueo no resuelto, en lugar de adivinar.
...
Un ser humano está presente en la sesión. El agente propone, el humano aprueba el plan materializado directamente, el agente ejecuta tarea por tarea, y la ambigüedad se resuelve preguntando. Todas las secciones del protocolo anteriores describen el perfil interactivo.
...
- **Plan preaprobado.** El plan fue aprobado antes de cualquier turno desatendido — en modo guiado el humano aprobó el plan materializado directamente; en modo trust la instrucción `trust` explícita del desarrollador es la aprobación registrada. Un agente NO DEBE crear y ejecutar un plan de forma desatendida en un mismo turno sin una u otra; la aprobación del plan es el punto de control humano.
