...
**Wersja 1.3.** Ten protokół definiuje, jak agent kodujący AI MUSI się zachowywać podczas pracy z Deep Work Plans. Słowa kluczowe MUST (MUSI), SHOULD (POWINIEN) i MAY (MOŻE) zgodne są z RFC 2119.
...
- Zapisać każde zadanie zgodnie z dziesięciosekcyjną anatomią — Powierzchnia dotknięta zapisuje, co zadanie zmienia, i wybiera jego bramkę walidacyjną według klasy ryzyka.
- Zakończyć pojedynczym obowiązkowym Final Review (przegląd bezpieczeństwa, walidacja stanu końcowego, uzgodnienie decyzji dotyczących skilli).
...
- Przechodzić od zaliczonej bramki do następnego zadania bez pytania o potwierdzenie; przy niezdanej bramce podejmować naprawy w ramach autoryzowanego zakresu zadania — z konkretną hipotezą, przerywając ślepe powtórzenia po dwóch próbach bez nowych dowodów.
- Zatrzymywać się i pytać tylko przy rzeczywistej granicy zatwierdzenia albo nierozwiązanym blokerze, zamiast zgadywać.
...
W sesji obecny jest człowiek. Agent proponuje, człowiek bezpośrednio zatwierdza zmaterializowany plan, agent wykonuje zadanie po zadaniu, a niejednoznaczności są rozwiązywane przez pytanie. Wszystkie powyższe sekcje protokołu opisują profil interaktywny.
...
- **Wstępnie zatwierdzony plan.** Plan został zatwierdzony przed jakąkolwiek turą nieobsługiwaną — w trybie guided człowiek bezpośrednio zatwierdził zmaterializowany plan; w trybie trust jawna instrukcja `trust` programisty jest zapisanym zatwierdzeniem. Agent NIE MOŻE bez żadnego z nich tworzyć i wykonywać planu nieobsługiwanego w jednej turze; zatwierdzenie planu jest ludzkim punktem kontrolnym.
