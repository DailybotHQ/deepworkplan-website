...
- **Modalità guidata (predefinita).** Il flusso produce esattamente **un** artefatto da revisionare: una **bozza raffinata** in `.dwp/drafts/`, con struttura sufficiente per approvare o chiedere modifiche in un solo passaggio. La cartella del piano è materializzata solo dopo l'approvazione.
- **Modalità trust (`trust` / `auto`).** Il flusso **materializza il piano direttamente**, senza file di bozza — lo sviluppatore ha rinunciato alla revisione intermedia. L'analisi dei requisiti, l'ordinamento delle dipendenze e un **controllo di qualità del piano** vengono comunque eseguiti: trust rinuncia alla *revisione*, non all'*analisi*. Un piano in modalità trust è registrato come **pre-approvato per l'esecuzione non presidiata**.
- **Modalità di bozza esplicite.** `refined-draft {name}` e `from-refined-draft {file}` restano disponibili in entrambe le modalità; uno sviluppatore che chiede una bozza la ottiene.

In ogni modalità il flusso non produce mai una bozza intermedia non raffinata come passo revisionabile separato.
