...
`state.json` MUST перезаписуватися агентом у кожній із цих точок протоколу: матеріалізація плану (всі завдання `pending`), початок завдання (`in_progress`), кожен запуск валідаційного gate (запис gate додається або оновлюється) та завершення завдання (`completed`, як частина протоколу завершення завдання в [Специфікації DWP](/spec/dwp-specification#task-completion-protocol)), контрольна точка перед будь-яким запланованим перериванням і зупинка `blocked`.
