---
title: Додатки
description: "Аддони DWP: чотири опційні розширення, обовʼязковий локальний огляд AI Diff Reviewer з опційною CI-поверхнею, контракт аддона та поняття kit."
order: 6
lang: uk
section: Addons
---
...
**[AI Diff Reviewer](https://github.com/DailybotHQ/ai-diff-reviewer)** (marketplace **«AI Diff Reviewer»**) надає обовʼязковій перевірці безпеки Final Review структурований локальний огляд і необовʼязково контролює pull request у CI. Починаючи зі стандарту 2.3.0 **локальний огляд є частиною базового рівня**; опційною лишається лише CI-поверхня. Цей аддон автоматично оновлюється з кожним релізом, тож показаний нижче тег є чинним на момент написання і може відставати від vendored-копії — джерелом істини про фактично встановлений тег є власний `SKILL.md` аддона та його релізи на GitHub. Встановлення завжди закріплюється за опублікованим тегом, а не за рухомою гілкою.

- **Сторінка kit:** [AI Diff Reviewer](/kit/ai-diff-reviewer) — повна довідка можливостей
- **Обовʼязково під час онбордингу (Phase 7a):** закріплене за тегом встановлення vendored-скіла (`npx --yes skills add DailybotHQ/ai-diff-reviewer@v3.1.1 --skill ai-diff-reviewer -y`) плюс адаптований під репозиторій `.review/extension.md` (через `generate-extension`) за згодою онбордингу; цільове оновлення harness узгоджує обидва, коли вони відсутні; відмова записується як заявлений виняток і про неї звітує `verify`, доки оглядач не встановлено
- **Обовʼязково в кожному Final Review:** перевірка безпеки запускає типовий батьківський потік upstream над накопиченим набором змін і додає його вивід до локального для плану `analysis_results/SECURITY_REVIEW.md` (усередині власної теки плану, ніколи в корені репозиторія); відсутній скіл або розширення — це записана знахідка `local reviewer not installed` — ніколи не мовчазний пропуск і ніколи не несподіваний бустреп: встановлення належить згоді онбордингу або явному виклику адд-она; **підтверджені критичні знахідки** завершеного проходу блокують завершення, доки їх не виправлено або явно не прийнято (v3, BC-07 — непідтверджені критичні твердження надходять як анотовані попередження, а огляд `incomplete`/`timeout` не є чистим проходом, BC-04)
- **Опційна CI-поверхня (Flow B):** `pr-review.yml` (`DailybotHQ/ai-diff-reviewer@v3`) через upstream sub-skill `setup` плюс супутники `apply-review` (лише читання) та `address-review` (робить коміти, push і перевзводить оглядача; новий у v3.1.1) як зручності, які викликає розробник — пропонується явно, ніколи не встановлюється без запиту, ніколи не за замовчуванням, ніколи не як завдання плану
