...
export const ja: SiteTranslations = {
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          {
            id: 'multi-repo',
            question: '1 つの計画で複数のリポジトリをまたげますか？',
            answer:
    ...
    dwpStrengths: {
      ...
      items: [
        ...
        },
        {
          ...
          body: 'コミットされたスクリプトがフローごとに二つの測定値——セッション開始時に読み込まれる入口バンドルと、実際のトリガーが発火した後のエンドツーエンドのパス——を、それぞれの除外事項とともに公開するため、入口の数値だけが実行全体のコストとして読まれることはありません。増加を含む結果は常にバイトとして公開され、トークンやコストのパーセンテージとして公開されることはありません。',
        },
      ],
    },
    honestLimits: {
      title: '正直な限界',
      body: 'Deep Work Plan には生きた仕様やデルタ仕様の仕組みがなく、その領域では OpenSpec などのツールが優れています。方法論の独立したベンチマークはまだ存在しません。一方、ファーストパーティによるフレッシュエージェント評価が凍結プロトコルの下で実行済みです——規模は小さく、単一ワークロード、構成ごとに2つの機能、1台のマシン——その結果は両方向で公開されています：ハーネスを持つツリー上のエージェントは両タスクでバイト読取量が少なく、現行版のセッションは前のメジャーバージョンより少ない、ハーネス報告のモデル入出力を消費した一方、ワークロードごとのトークンの純方向は混在し、ウォールクロックの優位は主張されていません。指示読み込みの台帳は読み込まれたバイト数を測るものであり、トークン、コスト、成果ではありません。また、その入口バンドルの数値は、実行が読み込む量の上限ではありません。DWP はあえてリポジトリの範囲に限定されています。プロジェクトをまたぐメモリシステムでも、役割ベースのエージェントフレームワークでも、IDE でもないため、これらの軸では競合しません——その能力が必要な作業には、それをカバーするツールと組み合わせてください。',
    },
    correction: {
  ...
  developersPage: {
    ...
    formatTitle: '二つの独立した選択:形式と、どれだけレビューを求めるか',
    formatIntro:
      'すべての計画は、それぞれの軸から一つの値を選びます。この二つの軸は独立しています——Lite の計画で trust を実行することも、Full の計画で guided を実行することもでき、どちらも形式を変えずに後からモードを切り替えられます。',
    formatAxes: [
      {
        title: 'Lite',
        body: 'タスク記録は計画の README 内に、安定した #task-N アンカーの下でインラインに存在します——タスクファイルを分けることはありません。小さく範囲の定まった作業向けです:一つの関心事を、おおよそ一回の作業時間で。それでも計画としては完全です:安定したタスク ID、Touched Surface、受け入れ基準、検証ゲート、そして Final Review を備えており、簡略化されたスケッチでは決してありません。',
      },
      {
        title: 'Full',
        body: 'N.task_<slug>.md の下にタスクごとに一つのファイルを持ちます。数時間から数日にわたる長期的な作業や、タスク間に実際の依存関係がある計画向けです。コンパクトな記録では足りなくなったとき、Lite の計画は後から /dwp-refine promote で Full に昇格できます——昇格によって完了済みの作業が再実行されることはありません。',
      },
      {
        title: 'Guided (default)',
        body: 'dwp-create は目標を分析し、分解して、レビュー可能な計画を具体化します——それは使い捨ての下書きではなく、すでに実際に実行可能な計画です——その上で、そのまま採用するか、Lite を Full に昇格するか、編集するか、停止するかを尋ねます。実際のプロダクト作業が始まる前に、人間がループに留まります。最初の数回や、より重要な作業には guided を推奨します。',
      },
      {
        title: 'Trust (or auto)',
        body: '最後の単語として trust(または auto)を付け加えます——例:/dwp-create <goal> trust——すると、エージェントはレビューの段階を省略し、事前承認済みの計画を具体化して、実行コマンドを直接返します。フローを信頼できるようになったパワーユーザー向けの近道です。あらゆる判断とゲートは引き続き記録されますが、確認のために立ち止まることはありません。',
      },
    ],
    commandsTitle: '九つのサブスキル',
    commandsIntro:
    ...
    commands: [
      {
        ...
        description:
          '目標を計画に変えます——デフォルトは Lite、より大きな作業には Full。どちらのモードも上の表の通りです。',
        href: '/kit/dwp-create',
      },
      {
        name: '/dwp-execute',
        description:
          '既存の計画をタスクごとに実行します:計画全体を読み、各タスクを順番に実行し、そのゲートを検証し、進捗を更新します。',
        href: '/kit/dwp-execute',
      },
      {
        name: '/dwp-refine',
        description:
          '完了した作業とその記録された証拠を保持したまま、既存の計画のタスクを追加・削除・並べ替えます。',
        href: '/kit/dwp-refine',
      },
      {
        name: '/dwp-resume',
        description:
          '計画自身のファイルから状態を再構築し、中断された計画を最初の未完了タスクから継続します。',
        href: '/kit/dwp-resume',
      },
      {
        name: '/dwp-status',
        description:
          '計画の進捗——完了・進行中・保留中のタスク——を、何も変更せずに報告します。',
        href: '/kit/dwp-status',
      },
      {
        name: '/dwp-verify',
        description:
          'リポジトリが AI-first であるか、その計画群が適切な形式であるかを機械的にチェックします。何も変更せず、合格か不合格かを報告します。',
        href: '/kit/dwp-verify',
      },
      {
        name: '/deepworkplan-onboard',
        description:
          'リポジトリを AI-first にします:そのスタックについて推論した上で、適応した AGENTS.md、docs/、.agents/、そして gitignore された .dwp/ を生成します。',
        href: '/kit/deepworkplan-onboard',
      },
      {
        name: '/skill-create, /agent-create',
        description:
          '作者向けのサブスキルです:リポジトリ自身のキットを育てます——繰り返し可能な手順のための再利用可能なスキル、または独自のモデルとツールを持つ繰り返しの役割のためのエージェントです。',
        href: '/kit/skill-create',
      },
      {
        name: '/dwp-upgrade',
        description:
          '公開されている新しいスキルのリリースを確認し、明示的な承認があった場合にのみそれをインストールして、オンボーディングを新規のパスとして再実行します——.dwp/ の下にある進行中の計画はすべて手つかずのまま残ります。',
        href: '/kit',
      },
    ],
    examplesTitle: '実行する二つの方法',
    examplesIntro:
      '同じスキル、同じ九つのコマンド——変わるのはツールではなく、作業の規模とリスクに応じた形式とレビューモードです。',
    quickFixLabel: '小さく範囲の定まった修正——Lite、trust',
    quickFixBody:
      '一つの関心事を、おおよそ一回の作業時間で、リスクは低い:レビューの段階を省略し、エージェントに Lite の計画を直接具体化・実行させます。',
    longHorizonLabel: '長期にわたる作業——Full、guided',
    longHorizonBody:
      'タスク間に実際の依存関係がある、あるいはリスクがより高い場合:プロダクトの作業が始まる前に提案された計画をレビューし、目標に応じて必要なら Full に昇格し、その後は必要に応じてセッションをまたいで実行・再開します。',
    planCodeLabel: 'ターミナル——スラッシュコマンド',
    planNote:
      'すべての計画の出力——マニフェスト、進捗ログ、タスク記録、ゲートの証拠——は、リポジトリ自体の中にある gitignore された .dwp/ ディレクトリの下に置かれます。deepworkplan.com に送信されたり保存されたりすることは一切なく、このスキルはネットワーク呼び出しを一切行いません。',
