...
export const ru: SiteTranslations = {
  ...
  announcementBar: {
    badge: 'Новое · Экономичен по контексту по своей конструкции',
  ...
  home: {
    ...
    story: {
      ...
      act5: {
        kicker: 'Метод · Акт V',
        lead: 'Контекст — самый дефицитный ресурс агента. Harness загружается прогрессивно, проверяет только изменённое и учится на месте каждой задачи — нагрузка инструкций каждого потока измерена и опубликована, релиз за релизом.',
        deepLinkLabel: 'Читать методологию',
        deepLinkHref: '/methodology',
      },
    },
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            linkLabel: 'Контракт соответствия',
      ...
      },
      {
        ...
        items: [
          {
            id: 'multi-repo',
            question: 'Может ли один план охватывать несколько репозиториев?',
            answer:
            ...
            linkLabel: 'Архетипы репозиториев',
            linkPath: '/spec/archetypes',
          },
          {
            id: 'vs-sdd-tools',
  ...
  developersPage: {
    ...
    planTitle: 'Планируйте и выполняйте с помощью skill',
    planIntro:
      'API выше позволяет агенту читать этот сайт. Skill DWP — это то, что позволяет агенту выполнять методологию: установите её один раз в репозитории, и она принесёт роутер плюс девять sub-skills, вызываемых как слэш-команды (или по имени — для агентов, перехватывающих слэш, большинство использует # вместо этого, например #dwp-execute).',
    formatTitle: 'Два независимых выбора: формат и сколько ревью вы хотите',
    formatIntro:
      'Каждый план выбирает одно значение по каждой оси. Они независимы — план Lite может работать в trust, план Full может работать в guided, и любой из них может переключить режим позже, не меняя формат.',
    formatAxes: [
      {
        title: 'Lite',
        body: 'Записи задач живут прямо в README плана, за стабильными якорями #task-N — без отдельных файлов задач. Создан для небольшой, ограниченной работы: одна забота, примерно одна сессия. Всё же полноценный план: стабильные id задач, Touched Surface, критерии приёмки, validation gate и Final Review — никогда не урезанный набросок.',
      },
      {
        title: 'Full',
        body: 'Один файл на задачу под N.task_<slug>.md — для работы на долгую перспективу, растянутой на часы или дни, или когда у плана есть реальные зависимости между задачами. План Lite позже повышается до Full командой /dwp-refine promote, когда компактных записей перестаёт хватать — повышение никогда не переисполняет уже завершённую работу.',
      },
      {
        title: 'Guided (по умолчанию)',
        body: 'dwp-create анализирует цель, декомпозирует её и материализует план, готовый к ревью — уже реальный, исполняемый план, никогда не черновой набросок, — а затем спрашивает: оставить его, повысить Lite до Full, отредактировать или остановиться. Человек остаётся в цикле до начала любой продуктовой работы. Рекомендуется в первые несколько раз или для чего-то с более высокими ставками.',
      },
      {
        title: 'Trust (или auto)',
        body: 'Добавьте trust (или auto) последним словом — например /dwp-create <goal> trust — и агент пропускает раунд ревью, материализует заранее одобренный план и сразу возвращает команду выполнения. Ярлык для опытных пользователей, когда вы уже доверяете потоку; он всё равно записывает каждое решение и gate, просто не останавливается, чтобы спросить.',
      },
    ],
    commandsTitle: 'Девять sub-skills',
    commandsIntro:
      'Каждая sub-skill вызывается как слэш-команда внутри репозитория, установившего skill, — не против этого сайта. Полное описание каждой живёт в каталоге кита.',
    commands: [
      {
        name: '/dwp-create <goal>',
        description:
          'Превращает цель в план — Lite по умолчанию, Full для более крупной работы, любой из режимов таблицы выше.',
        href: '/kit/dwp-create',
      },
      {
        name: '/dwp-execute',
        description:
          'Выполняет существующий план задача за задачей: читает его полностью, выполняет каждую задачу по порядку, валидирует её gate, обновляет прогресс.',
        href: '/kit/dwp-execute',
      },
      {
        name: '/dwp-refine',
        description:
          'Добавляет, удаляет или переупорядочивает задачи в существующем плане, сохраняя завершённую работу и её зафиксированные свидетельства.',
        href: '/kit/dwp-refine',
      },
      {
        name: '/dwp-resume',
        description:
          'Восстанавливает состояние из собственных файлов плана и продолжает прерванный план с первой незавершённой задачи.',
        href: '/kit/dwp-resume',
      },
      {
        name: '/dwp-status',
        description:
          'Сообщает о прогрессе плана — завершённые, выполняемые, ожидающие задачи — не внося никаких изменений.',
        href: '/kit/dwp-status',
      },
      {
        name: '/dwp-verify',
        description:
          'Механически проверяет, является ли репозиторий AI-first и хорошо ли сформированы его планы. Ничего не меняет; сообщает pass или fail.',
        href: '/kit/dwp-verify',
      },
      {
        name: '/deepworkplan-onboard',
        description:
          'Делает репозиторий AI-first: анализирует его стек, затем генерирует адаптированный AGENTS.md, docs/, .agents/ и игнорируемый git .dwp/.',
        href: '/kit/deepworkplan-onboard',
      },
      {
        name: '/skill-create, /agent-create',
        description:
          'Sub-skill для авторства: развивает собственный кит репозитория — переиспользуемую skill для повторяемой процедуры или агента для повторяющейся роли со своей моделью и инструментами.',
        href: '/kit/skill-create',
      },
      {
        name: '/dwp-upgrade',
        description:
          'Проверяет наличие более нового опубликованного релиза skill и, только после явного одобрения, устанавливает его и заново запускает onboarding как свежий проход — каждый выполняющийся план под .dwp/ остаётся нетронутым.',
        href: '/kit',
      },
    ],
    examplesTitle: 'Два способа запустить это',
    examplesIntro:
      'Одна и та же skill, одни и те же девять команд — формат и режим ревью меняются вместе с размером и ставками работы, а не с инструментом.',
    quickFixLabel: 'Маленькое, ограниченное исправление — Lite, trust',
    quickFixBody:
      'Одна забота, примерно одна сессия, низкие ставки: пропустите раунд ревью и позвольте агенту материализовать и сразу выполнить план Lite.',
    longHorizonLabel: 'Работа на долгую перспективу — Full, guided',
    longHorizonBody:
      'Реальные зависимости между задачами или более высокие ставки: просмотрите предложенный план до начала любой продуктовой работы, повысьте до Full, если цель окажется требующей этого, затем выполняйте и возобновляйте между сессиями по мере необходимости.',
    planCodeLabel: 'Терминал — слэш-команды',
    planNote:
    ...
    endpoints: [
      ...
      },
      {
        ...
        description:
          'Версионируемый каталог семейства v1: пути конечных точек, версия сайта и ссылки на спецификацию.',
      },
      {
        method: 'GET',
        path: '/api/v1/sections.json',
        description:
          'Карта сайта в виде типизированного JSON — имя, путь и описание для каждого раздела.',
      },
      {
        method: 'GET',
        path: '/api/v1/pages.json',
        description:
          'Каждая Markdown-конечная точка на каждом языке, сгруппированные по коду языка.',
      },
      {
        method: 'GET',
        path: '/api/v1/health.json',
        description:
          'Версионируемый маркер состояния — v1-зеркало /api/health.json.',
      },
      {
        method: 'POST',
        path: '/api/mcp',
    ...
    versioningTitle: 'Версионирование и устаревание',
    versioningBody:
