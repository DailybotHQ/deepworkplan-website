      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer:
              'Методология и навык распространяются под лицензией MIT и бесплатны; в основных потоках нет сервиса, API-ключа и телеметрии. Эффективность сообщается как число байтов инструкций, загружаемых каждым потоком: её измеряет зафиксированный вместе с навыком скрипт, перемеряет на каждой базовой линии релизов и публикует в реестре оценки, причём рост сообщается так же прямо, как и снижение. Процентами токенов или экономией затрат она не сообщается, потому что инвентаризация байтов этого не устанавливает; для корректного измерения результатов запланирована предварительно зарегистрированная публичная оценка.',
