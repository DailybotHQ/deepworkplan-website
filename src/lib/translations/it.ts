...
export const it: SiteTranslations = {
  ...
  announcementBar: {
    badge: 'Novità · Efficiente nel contesto per design',
  ...
  home: {
    ...
    hero: {
      ...
      copiedLabel: 'Copiato',
      viewInitCta: 'Scopri come funziona',
      pullQuote:
  ...
  kitPage: {
    meta: {
      ...
      description:
        'La skill e le sue nove sub-skill, i comandi, gli adapter, i preset, gli addon opt-in e gli esempi che rendono Deep Work Plan eseguibile su agenti e stack diversi.',
    },
    eyebrow: 'Kit',
    title: 'Il kit',
    intro:
    ...
    groups: {
      command: {
        ...
        description:
          'Il router della skill e le sue sub-skill (create, execute, refine, resume, status, verify, onboard, author, upgrade), più i sottili slash command che vi delegano.',
      },
      adapter: {
        title: 'Adapter',
        description:
    ...
    groups: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer:
              'Una scelta di rappresentazione, non un compromesso sul rigore. I piani sono Lite per impostazione predefinita: un README compatto con record di task ancorati che è già eseguibile, non una bozza parziale. Se si chiede un piano Full fin dall’inizio, `create` scrive direttamente i file di task Full; ed espande un piano a Full quando il dettaglio delle istruzioni, le dipendenze o i contratti di un task non rientrano più in un record compatto e revisionabile. La promozione successiva conserva ogni task completato. Entrambi i formati portano gli stessi criteri di accettazione, validation gate, evidenze e il Final Review obbligatorio.',
            linkLabel: 'Leggi la metodologia',
            linkPath: '/methodology',
          },
          {
        ...
        items: [
          {
            ...
            answer:
              'Sono asserzioni eseguibili che l’agente esegue da sé. L’approvazione umana delimita l’esecuzione all’inizio e alla fine: una persona approva il piano prima dell’esecuzione e rivede il diff finale in fase di pull request; l’esecuzione nel mezzo è autonoma. Ogni task indica comandi concreti, in genere il quality gate del repository stesso, selezionati dalla superficie toccata dal task: i test del comportamento modificato e dei suoi consumer, ampliati alla suite completa quando la modifica è condivisa o non delimitabile. Un task viene marcato come fatto solo quando quei comandi terminano con successo, e i task che modificano il comportamento devono estendere i test. In caso di fallimento l’agente ripara prima ciò che ricade nell’ambito proprio della task e riesegue il gate; un fallimento non riparabile in quell’ambito lascia il task marcato come bloccato e ferma l’esecuzione.',
          ...
          },
          {
            ...
            answer:
              'Un gate fallito è prima di tutto un segnale di riparazione: l’agente corregge ciò che ricade nell’ambito proprio della task e riesegue il gate. Un fallimento che eccede quell’ambito lascia il task registrato come bloccato, e l’agente si ferma prima di dichiarare il completamento. Lei può ispezionare le evidenze, riparare il codice o rifinire il task, quindi riprendere; un comando fallito è un segnale per risolvere la discrepanza, non un permesso per indebolire il gate.',
          ...
          },
          {
            id: 'unverified',
            question:
              'Cosa succede quando il verificatore di conformità non può eseguire i suoi controlli?',
            answer:
              'Lo dice chiaramente. Il verificatore termina con codice di uscita 2 e un verdetto `UNVERIFIED` esplicito — non stampa mai un esito positivo che non abbia effettivamente verificato. Se l’ambiente manca di un interprete capace o un controllo non può girare, l’esito onesto è «non verificato», non «conforme»; un risultato verde significa sempre che ogni controllo è stato eseguito ed è passato. La stessa disciplina attraversa tutta la metodologia: nessun flusso indebolisce o falsifica un gate per dichiarare il lavoro completato.',
            linkLabel: 'Il contratto di conformità',
      ...
      },
      {
        ...
        items: [
          {
            id: 'multi-repo',
            question: 'Un piano può estendersi su più repository?',
            answer:
              'Sì — l’archetipo dell’hub orchestratore esiste esattamente per questo. Un repository hub tiene il piano che coordina, e ogni repository figlio esegue il proprio piano dentro la propria area `.dwp/` isolata, quindi un figlio non scrive mai nello stato dei piani dell’hub. Il completamento del figlio si legge dallo stato di livello superiore del piano stesso, non cercando stringhe al suo interno, e l’hub registra dove si trova prima di navigare da qualsiasi parte. Ogni figlio resta un repository DWP ordinario, che può essere pilotato anche da solo.',
            linkLabel: 'Archetipi di repository',
            linkPath: '/spec/archetypes',
          },
          {
            id: 'vs-sdd-tools',
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            id: 'upgrade',
            question:
              'Come aggiorno la skill in un repository già sottoposto a onboarding?',
            answer:
  ...
  developersPage: {
    meta: {
      ...
      description:
        'La superficie per agenti di Deep Work Plan: API versionata, di sola lettura e senza autenticazione, con OpenAPI, server MCP e Markdown per pagina in 17 lingue.',
    },
    ...
    endpoints: [
      ...
      },
      {
        ...
        description:
          'Catalogo versionato della famiglia v1: percorsi degli endpoint, versione del sito e collegamenti alla specifica.',
      },
      {
        method: 'GET',
        path: '/api/v1/sections.json',
        description:
          'La mappa del sito come JSON tipizzato — nome, percorso e descrizione per sezione.',
      },
      {
        method: 'GET',
        path: '/api/v1/pages.json',
        description:
          'Tutti gli endpoint Markdown in ogni lingua, raggruppati per codice lingua.',
      },
      {
        method: 'GET',
        path: '/api/v1/health.json',
        description:
          'Indicatore di stato versionato — il mirror v1 di /api/health.json.',
      },
      {
        method: 'POST',
        path: '/api/mcp',
    ...
    versioningTitle: 'Versionamento e deprecazione',
    versioningBody:
    ...
    rateLimitsTitle: 'Limiti di richieste',
    rateLimitsBody:
    ...
    cliCodeLabel: 'Terminale — CLI per le skill e CLI ufficiale',
    cliNote:
