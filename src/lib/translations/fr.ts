...
export const fr: SiteTranslations = {
  ...
  announcementBar: {
    badge: 'Nouveau · Efficace en contexte par construction',
  ...
  home: {
    ...
    pitch: {
      ...
      efficiency:
        'Et parce que le contexte est la ressource la plus rare de votre agent, le harness est conçu pour garder sa propre charge d’instructions faible et auditable : les instructions se chargent progressivement, la validation ne touche que ce qui a changé, et chaque tâche apprend localement. La charge de chaque flux est mesurée en octets et publiée à chaque publication — la dernière publication a réduit le flux d’exécution de 45,7 % et le flux de reprise de 67,5 %. Le plan lui-même suit le même principe : un plan Lite pour une correction bornée, un plan Full pour un travail qui s’étend sur des heures — le format suit toujours l’ampleur du travail, jamais l’inverse.',
      illustrationAlt:
    ...
    story: {
      ...
      act5: {
        ...
        lead: 'Le contexte est la ressource la plus rare de votre agent. Le harness se charge progressivement, ne valide que ce qui a changé et apprend tâche par tâche — la charge d’instructions de chaque flux étant mesurée et publiée, publication après publication.',
        deepLinkLabel: 'Lire la méthodologie',
        deepLinkHref: '/methodology',
      },
    },
    ...
    quickstart: {
      ...
      steps: [
        ...
        },
        {
          ...
          description:
            'Le skill est le moteur — le même dans chaque dépôt. Une seule commande importe le routeur et ses sous-skills (create, execute, refine, resume, status, verify, onboard, author, upgrade) pour Claude Code, Cursor, Codex, Gemini et Copilot.',
        },
        {
    ...
    outcomes: {
      ...
      items: [
        ...
        },
        {
          ...
          detail:
            'create, execute, refine, resume, status, verify, onboard, author et upgrade — disponibles pour votre agent sous forme d’un seul pack de skills, sans copie par dépôt.',
        },
        {
  ...
  kitPage: {
    meta: {
      ...
      description:
        'Le skill et ses neuf sous-skills, commandes, adaptateurs, presets, addons facultatifs et exemples qui rendent Deep Work Plan exécutable sur tous les agents et toutes les stacks.',
    },
    eyebrow: 'Kit',
    title: 'Le kit',
    intro:
    ...
    groups: {
      command: {
        ...
        description:
          'Le routeur de skills et ses sous-skills (create, execute, refine, resume, status, verify, onboard, author, upgrade), ainsi que les commandes slash légères qui leur délèguent.',
      },
      adapter: {
        title: 'Adaptateurs',
        description:
    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            linkLabel: 'Le contrat de conformité',
      ...
      },
      {
        ...
        items: [
          {
            id: 'multi-repo',
            question: 'Un plan peut-il s’étendre sur plusieurs dépôts ?',
            answer:
            ...
            linkLabel: 'Archétypes de dépôts',
            linkPath: '/spec/archetypes',
          },
          {
            id: 'vs-sdd-tools',
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer:
              'La méthodologie et le skill sont sous licence MIT et gratuits ; il n’y a ni service, ni clé d’API, ni télémétrie dans les flux centraux. L’efficacité est rapportée comme le nombre d’octets d’instructions que chaque flux charge **à l’entrée** — son paquet de départ au début d’une session — publié aux côtés de **parcours de bout en bout** nommés, qui ajoutent ce que les déclencheurs propres du flux chargent une fois le travail réel engagé (par exemple, une reprise qui va jusqu’à l’exécution charge typiquement plusieurs fois son paquet d’entrée). Aucun des deux chiffres ne plafonne une session : une exécution réelle lit aussi les fichiers propres du dépôt, la sortie des outils et les fichiers de travail du plan, dont aucun n’est compté par ce registre. Les deux nombres sont mesurés par un script livré avec le skill, remesuré à chaque ligne de base de publication et publié dans un registre d’évaluation, les hausses étant rapportées aussi platement que les baisses. Elle n’est pas rapportée en pourcentages de tokens ni en économies de coût, parce qu’un inventaire d’octets n’établit pas ces choses. Une évaluation publique à agents frais s’est déjà déroulée sous un protocole figé : les deux mêmes fonctionnalités construites depuis des clones propres sans harness, avec la version majeure précédente et avec l’actuelle. Elle a constaté que les agents sur un arbre porteur de harness lisaient moins d’octets dans les deux tâches, et que les sessions de fonctionnalité de la version actuelle consommaient moins d’entrée et de sortie de modèle que celles de la version majeure précédente dans les deux tâches — telles que rapportées par le harness, sur une seule charge de travail. Elle a aussi établi ses limites honnêtes : l’onboarding est un coût unique qui ne se rembourse que si les flux sont utilisés, la direction nette des tokens par charge de travail était mitigée, aucun avantage de temps d’horloge n’est revendiqué, et un agent frais n’entre pas dans les flux de lui-même — les flux sont des commandes que vous, ou un agent qui les connaît, invoquez.',
  ...
  comparePage: {
    ...
    capabilities: {
      ...
      crossProjectMemory: {
        label: 'Mémoire d’agent persistante entre les projets',
        help: 'La mémoire suit l’agent d’un dépôt et d’une session à l’autre, et non le seul état sur disque d’un plan.',
      },
      roleBasedAgents: {
        label: 'Rôles d’agent spécialisés et distincts',
        help: 'Des personas nommés (analyste, architecte ou relecteur, par exemple) se répartissent le travail, au lieu qu’un seul agent exécute chaque étape.',
      },
      nativeIdeProduct: {
        label: 'Est lui-même un IDE ou un éditeur',
        help: 'L’outil est un environnement de développement intégré à part entière, pas un ajout à un agent de code déjà existant.',
      },
    },
    alternatives: {
      dwp: {
    ...
    aiNative: {
      title: 'Où cela s’inscrit dans l’AI-native SDLC',
      body: 'Le playbook AI-native SDLC de Claude décrit un cycle complet : Plan, Design, Build, Test, Deploy et Maintain. Chaque étape commite un artefact que l’étape suivante lit, avec une approbation humaine obligatoire à chaque étape et une passe de sécurité dédiée avant le déploiement, tandis que le feedback de production devient une nouvelle intention.',
      shared:
        'DWP partage les idées d’artefact durable et d’approbation par portes : l’intention devient un plan, les tâches laissent des preuves, une passe de sécurité s’exécute avant l’achèvement, et le dépôt reste lisible par l’agent suivant.',
      boundary:
        'La vraie différence porte sur le périmètre, pas sur la rigueur : le playbook est construit spécifiquement autour de Claude Code, tandis que le harness et le format de plan de DWP peuvent être lus par tout agent qui suit les standards AGENTS.md et Agent Skills. Le playbook couvre aussi l’évaluation continue et les opérations de production que DWP ne revendique pas ; ces pratiques opérationnelles peuvent compléter un dépôt piloté par DWP plutôt que lui faire concurrence.',
      sourceLabel: 'Lire le playbook AI-native SDLC de Claude',
    },
  ...
  developersPage: {
    ...
    planTitle: 'Planifier et exécuter avec la skill',
    planIntro:
      'L’API ci-dessus permet à un agent de lire ce site. La skill DWP est ce qui permet à un agent d’exécuter la méthodologie — installez-la une fois dans un dépôt, et elle fournit un routeur plus neuf sub-skills, invoquées comme des commandes slash (ou par leur nom, pour les agents qui interceptent le slash — la plupart utilisent # à la place, par exemple #dwp-execute).',
    formatTitle:
      'Deux choix indépendants : le format, et le niveau de relecture souhaité',
    formatIntro:
      'Chaque plan choisit une valeur sur chaque axe. Ils sont indépendants — un plan Lite peut fonctionner en trust, un plan Full peut fonctionner en guided, et l’un ou l’autre peut changer de mode plus tard sans changer de format.',
    formatAxes: [
      {
        title: 'Lite',
        body: 'Les enregistrements de tâches vivent en ligne dans le README du plan, derrière des ancres stables #task-N — pas de fichiers de tâche séparés. Conçu pour un travail petit et borné : une seule préoccupation, environ une session. Toujours un plan complet : identifiants de tâche stables, une Touched Surface, des critères d’acceptation, une porte de validation et un Final Review — jamais une esquisse allégée.',
      },
      {
        title: 'Full',
        body: 'Un fichier par tâche sous N.task_<slug>.md, pour un travail à long horizon s’étendant sur des heures ou des jours, ou lorsqu’un plan a de vraies dépendances entre tâches. Un plan Lite est promu en Full plus tard avec /dwp-refine promote lorsque les enregistrements compacts ne suffisent plus — la promotion ne ré-exécute jamais le travail terminé.',
      },
      {
        title: 'Guided (par défaut)',
        body: 'dwp-create analyse l’objectif, le décompose et matérialise un plan relisible — déjà le plan réel et exécutable, jamais une ébauche jetable — puis demande : le conserver, promouvoir Lite en Full, le modifier, ou arrêter. Un humain reste dans la boucle avant que tout travail produit ne commence. Recommandé les premières fois, ou pour tout ce qui comporte des enjeux plus élevés.',
      },
      {
        title: 'Trust (ou auto)',
        body: 'Ajoutez trust (ou auto) comme dernier mot — par exemple /dwp-create <goal> trust — et l’agent saute le tour de relecture, matérialise un plan pré-approuvé, et renvoie directement la commande d’exécution. Le raccourci pour utilisateur expérimenté une fois que vous faites confiance au flux ; il enregistre toujours chaque décision et chaque porte, il ne s’arrête simplement pas pour demander.',
      },
    ],
    commandsTitle: 'Les neuf sub-skills',
    commandsIntro:
      'Chaque sub-skill est invoquée comme une commande slash à l’intérieur du dépôt qui a installé la skill — pas contre ce site web. La référence complète de chacune se trouve dans le catalogue du kit.',
    commands: [
      {
        name: '/dwp-create <goal>',
        description:
          'Transforme un objectif en plan — Lite par défaut, Full pour un travail plus important, l’un ou l’autre mode selon le tableau ci-dessus.',
        href: '/kit/dwp-create',
      },
      {
        name: '/dwp-execute',
        description:
          'Exécute un plan existant tâche par tâche : le lit intégralement, exécute chaque tâche dans l’ordre, valide sa porte, met à jour la progression.',
        href: '/kit/dwp-execute',
      },
      {
        name: '/dwp-refine',
        description:
          'Ajoute, supprime ou réordonne des tâches dans un plan existant tout en préservant le travail terminé et ses preuves enregistrées.',
        href: '/kit/dwp-refine',
      },
      {
        name: '/dwp-resume',
        description:
          'Reconstruit l’état à partir des propres fichiers du plan et poursuit un plan interrompu depuis sa première tâche incomplète.',
        href: '/kit/dwp-resume',
      },
      {
        name: '/dwp-status',
        description:
          'Rapporte la progression d’un plan — tâches terminées, en cours, en attente — sans effectuer aucun changement.',
        href: '/kit/dwp-status',
      },
      {
        name: '/dwp-verify',
        description:
          'Vérifie, mécaniquement, si le dépôt est AI-first et si ses plans sont bien formés. Ne change rien ; rapporte réussite ou échec.',
        href: '/kit/dwp-verify',
      },
      {
        name: '/deepworkplan-onboard',
        description:
          'Rend un dépôt AI-first : raisonne sur sa stack, puis génère un AGENTS.md adapté, docs/, .agents/, et un .dwp/ ignoré par git.',
        href: '/kit/deepworkplan-onboard',
      },
      {
        name: '/skill-create, /agent-create',
        description:
          'La sub-skill auteure : fait grandir le kit propre du dépôt — une skill réutilisable pour une procédure répétable, ou un agent pour un rôle récurrent avec son propre modèle et ses propres outils.',
        href: '/kit/skill-create',
      },
      {
        name: '/dwp-upgrade',
        description:
          'Vérifie s’il existe une version plus récente de la skill publiée et, seulement après approbation explicite, l’installe et relance l’intégration comme un passage neuf — chaque plan en cours sous .dwp/ reste intact.',
        href: '/kit',
      },
    ],
    examplesTitle: 'Deux façons de l’exécuter',
    examplesIntro:
      'La même skill, les mêmes neuf commandes — le format et le mode de relecture changent avec la taille et les enjeux du travail, pas avec l’outil.',
    quickFixLabel: 'Une correction petite et bornée — Lite, trust',
    quickFixBody:
      'Une seule préoccupation, environ une session, enjeux faibles : sauter le tour de relecture et laisser l’agent matérialiser et exécuter directement un plan Lite.',
    longHorizonLabel: 'Travail à long horizon — Full, guided',
    longHorizonBody:
      'De vraies dépendances entre tâches, ou des enjeux plus élevés : relire le plan proposé avant que tout travail produit ne commence, le promouvoir en Full si l’objectif s’avère en avoir besoin, puis exécuter et reprendre entre sessions au besoin.',
    planCodeLabel: 'Terminal — commandes slash',
    planNote:
    ...
    endpoints: [
      ...
      },
      {
        ...
        description:
          'Catalogue versionné de la famille v1 : chemins des endpoints, version du site et liens vers la spécification.',
      },
      {
        method: 'GET',
        path: '/api/v1/sections.json',
        description:
          'Le plan du site en JSON typé — nom, chemin et description par section.',
      },
      {
        method: 'GET',
        path: '/api/v1/pages.json',
        description:
          'Tous les endpoints Markdown dans chaque langue, groupés par code de langue.',
      },
      {
        method: 'GET',
        path: '/api/v1/health.json',
        description:
          'Marqueur d’état versionné — le miroir v1 de /api/health.json.',
      },
      {
        method: 'POST',
        path: '/api/mcp',
    ...
    versioningTitle: 'Versionnement et dépréciation',
    versioningBody:
    ...
    rateLimitsTitle: 'Limites de débit',
    rateLimitsBody:
    ...
    cliCodeLabel: 'Terminal — CLI de skills et CLI officielle',
    cliNote:
      'La skill s’installe par vendoring dans .agents/skills/deepworkplan/ à l’intérieur de votre dépôt, pour que chaque agent qui touche le dépôt partage la même méthodologie. La CLI officielle deepworkplan — un client sans dépendances sur la même API (init, sections, read, open, mcp) — est prête pour npm et vit dans le répertoire cli/ du dépôt du site jusqu’à sa publication.',
