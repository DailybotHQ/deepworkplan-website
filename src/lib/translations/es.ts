...
export const es: SiteTranslations = {
  ...
  home: {
    ...
    story: {
      ...
      act5: {
        ...
        lead: 'El contexto es el recurso más escaso de tu agente. El harness carga progresivamente, valida lo que cambió y aprende tarea a tarea — con la carga de instrucciones de cada flujo medida y publicada, lanzamiento tras lanzamiento.',
        deepLinkLabel: 'Lee la metodología',
        deepLinkHref: '/methodology',
      },
    },
    onboarding: {
      ...
      steps: [
        ...
        },
        {
          ...
          description:
            'Conecta el skill de Deep Work Plan y crea la carpeta .dwp/ ignorada por git para planes, instala la revisión local obligatoria de AI Diff Reviewer y luego, de forma opcional, añade complementos como soporte de devcontainer.',
        },
      ],
    },
    quickstart: {
    ...
    outcomes: {
      ...
      items: [
        ...
        },
        {
          ...
          detail:
            'Sin demonios ni estado externo. Los planes quedan en una carpeta .dwp/ ignorada por git, y cualquier tarea se reanuda solo desde git, incluso tras un desbordamiento de contexto.',
        },
      ],
    },
    agents: {
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            answer:
              'La metodología y la skill tienen licencia MIT y son gratuitas; no hay servicio, clave de API ni telemetría en los flujos centrales. La eficiencia se reporta como el número de bytes de instrucción que carga cada flujo **al entrar** — su paquete de entrada al inicio de una sesión —, publicado junto con **rutas de extremo a extremo** con nombre propio que suman lo que los propios disparadores del flujo cargan cuando el trabajo real continúa (por ejemplo, un resume que continúa hacia execute suele cargar varias veces su paquete de entrada). Ninguna de las dos cifras limita una sesión: una ejecución real también lee los propios archivos del repositorio, la salida de las herramientas y los archivos de trabajo del plan, nada de lo cual cuenta este registro. Ambas cifras se miden con un script versionado junto con la skill, vuelto a medir en cada línea base de publicación y publicado en un registro de evaluación, con los aumentos reportados con la misma claridad que las disminuciones. No se reporta como porcentajes de tokens ni ahorros de costo, porque un inventario de bytes no establece eso. Ya se ejecutó una evaluación pública con agentes frescos bajo un protocolo congelado: los mismos dos features construidos desde clones limpios sin harness, con la versión mayor anterior y con la actual. Encontró que los agentes sobre un árbol con harness leyeron menos bytes en ambas tareas, y que las sesiones de feature de la versión actual consumieron menos entrada y salida de modelo que las de la versión mayor anterior en ambas tareas — según lo reportó el harness, en una sola carga de trabajo. También encontró los límites honestos: el onboarding es un costo de una sola vez que solo se recupera cuando los flujos se usan, la dirección neta de tokens por carga de trabajo fue mixta, no se reclama ninguna ventaja de tiempo de reloj, y un agente fresco no entra a los flujos por sí solo — los flujos son comandos que invocas tú o el agente.',
    ...
    dwpStrengths: {
      ...
      items: [
        ...
        },
        {
          ...
          body: 'Un script versionado publica dos mediciones por flujo — el paquete de entrada que carga al inicio de una sesión y la ruta de extremo a extremo una vez que se activan sus disparadores reales —, además de lo que excluye cada una, de modo que la cifra de entrada nunca se lea como el costo total de una ejecución. Los resultados, incluidos los aumentos, se publican como bytes, nunca como porcentajes de tokens o de costo.',
        },
      ],
    },
    honestLimits: {
      title: 'Límites honestos',
      body: 'Una skill de DWP también es una especificación: describe cómo debe ejecutar un agente un procedimiento, como crear un componente. Los planes de DWP también son especificaciones de cambio: definen el alcance, los criterios de aceptación, las puertas de validación y las evidencias. Esta fila se refiere a algo más concreto: una especificación canónica que describe el comportamiento actual del producto y se actualiza con deltas después de cada cambio. Actualmente DWP no mantiene esa especificación viva a nivel de producto para cada sistema adoptado; OpenSpec es más fuerte cuando ese ciclo de vida es la necesidad principal. Las capas se pueden combinar alimentando un plan de DWP con una especificación del producto o una propuesta delta. Todavía no existe un benchmark independiente de la metodología; ya se ejecutó una evaluación propia con agentes frescos bajo un protocolo congelado, a escala pequeña — una carga de trabajo, dos features por configuración, una máquina — y sus resultados se publican en ambas direcciones: los agentes sobre árboles con harness leyeron menos bytes en ambas tareas y las sesiones de la versión actual consumieron menos entrada y salida de modelo, reportada por el harness, que las de la versión mayor anterior, mientras que la dirección neta de tokens por carga de trabajo fue mixta y no se reclama ninguna ventaja de tiempo de reloj. El registro de carga de instrucciones mide bytes cargados, no tokens, costo ni resultados, y su cifra de paquete de entrada no limita lo que lee una ejecución. DWP se limita deliberadamente al repositorio: no es un sistema de memoria entre proyectos, ni un framework de agentes basado en roles, ni un IDE, así que tampoco compite en esos frentes; combínalo con una herramienta que cubra ese frente cuando el trabajo lo requiera.',
    },
    correction: {
  ...
  developersPage: {
    meta: {
      ...
      description:
        'La superficie para agentes de Deep Work Plan: API versionada de solo lectura sin autenticación, OpenAPI, servidor MCP y Markdown por página en 17 idiomas.',
    },
    ...
    planTitle: 'Planifica y ejecuta con la skill',
    planIntro:
      'La API de arriba permite a un agente leer este sitio. La skill DWP es lo que permite a un agente ejecutar la metodología — instálala una vez en un repositorio y proporciona un enrutador más nueve sub-skills, invocadas como comandos de barra (o por su nombre, para agentes que interceptan la barra — la mayoría usa # en su lugar, por ejemplo #dwp-execute).',
    formatTitle:
      'Dos decisiones independientes: el formato y cuánta revisión quieres',
    formatIntro:
      'Cada plan elige un valor de cada eje. Son independientes — un plan Lite puede ejecutarse en trust, un plan Full puede ejecutarse en guided, y cualquiera puede cambiar de modo más tarde sin cambiar de formato.',
    formatAxes: [
      {
        title: 'Lite',
        body: 'Los registros de tareas viven en línea en el README del plan, tras anclas estables #task-N — sin archivos de tarea separados. Pensado para trabajo pequeño y acotado: un solo asunto, aproximadamente una sesión. Aun así, un plan completo: ids de tarea estables, una Touched Surface, criterios de aceptación, una puerta de validación y un Final Review — nunca un boceto reducido.',
      },
      {
        title: 'Full',
        body: 'Un archivo por tarea bajo N.task_<slug>.md, para trabajo de largo horizonte que se extiende por horas o días, o cuando un plan tiene dependencias reales entre tareas. Un plan Lite se promueve a Full más tarde con /dwp-refine promote cuando los registros compactos dejan de ser suficientes — la promoción nunca reejecuta el trabajo terminado.',
      },
      {
        title: 'Guided (por defecto)',
        body: 'dwp-create analiza el objetivo, lo descompone y materializa un plan revisable — ya el plan real y ejecutable, nunca un boceto desechable — y luego pregunta: conservarlo, promover Lite a Full, editarlo o detenerse. Una persona permanece en el bucle antes de que empiece cualquier trabajo de producto. Recomendado las primeras veces, o para cualquier cosa de mayor riesgo.',
      },
      {
        title: 'Trust (o auto)',
        body: 'Añade trust (o auto) como última palabra — por ejemplo /dwp-create <goal> trust — y el agente se salta la ronda de revisión, materializa un plan preaprobado y devuelve directamente el comando de ejecución. El atajo para usuarios avanzados una vez que confías en el flujo; sigue registrando cada decisión y puerta, simplemente no se detiene a preguntar.',
      },
    ],
    commandsTitle: 'Las nueve sub-skills',
    commandsIntro:
      'Cada sub-skill se invoca como un comando de barra dentro del repositorio que instaló la skill — no contra este sitio web. La referencia completa de cada una vive en el catálogo del kit.',
    commands: [
      {
        name: '/dwp-create <goal>',
        description:
          'Convierte un objetivo en un plan — Lite por defecto, Full para trabajo más grande, cualquiera de los modos según la tabla de arriba.',
        href: '/kit/dwp-create',
      },
      {
        name: '/dwp-execute',
        description:
          'Ejecuta un plan existente tarea por tarea: lo lee por completo, ejecuta cada tarea en orden, valida su puerta, actualiza el progreso.',
        href: '/kit/dwp-execute',
      },
      {
        name: '/dwp-refine',
        description:
          'Añade, elimina o reordena tareas en un plan existente preservando el trabajo completado y su evidencia registrada.',
        href: '/kit/dwp-refine',
      },
      {
        name: '/dwp-resume',
        description:
          'Reconstruye el estado a partir de los propios archivos del plan y continúa un plan interrumpido desde su primera tarea incompleta.',
        href: '/kit/dwp-resume',
      },
      {
        name: '/dwp-status',
        description:
          'Informa del progreso de un plan — tareas completadas, en curso, pendientes — sin hacer ningún cambio.',
        href: '/kit/dwp-status',
      },
      {
        name: '/dwp-verify',
        description:
          'Comprueba, mecánicamente, si el repositorio es AI-first y si sus planes están bien formados. No cambia nada; informa de aprobado o no aprobado.',
        href: '/kit/dwp-verify',
      },
      {
        name: '/deepworkplan-onboard',
        description:
          'Hace que un repositorio sea AI-first: razona sobre su stack y luego genera un AGENTS.md adaptado, docs/, .agents/ y un .dwp/ ignorado por git.',
        href: '/kit/deepworkplan-onboard',
      },
      {
        name: '/skill-create, /agent-create',
        description:
          'La sub-skill autora: hace crecer el kit propio del repositorio — una skill reutilizable para un procedimiento repetible, o un agente para un rol recurrente con su propio modelo y herramientas.',
        href: '/kit/skill-create',
      },
      {
        name: '/dwp-upgrade',
        description:
          'Comprueba si hay una versión más reciente de la skill publicada y, solo tras aprobación explícita, la instala y vuelve a ejecutar la incorporación como un pase nuevo — cada plan en curso bajo .dwp/ queda intacto.',
        href: '/kit',
      },
    ],
    examplesTitle: 'Dos formas de ejecutarla',
    examplesIntro:
      'La misma skill, los mismos nueve comandos — el formato y el modo de revisión cambian con el tamaño y el riesgo del trabajo, no con la herramienta.',
    quickFixLabel: 'Una corrección pequeña y acotada — Lite, trust',
    quickFixBody:
      'Un solo asunto, aproximadamente una sesión, bajo riesgo: saltarse la ronda de revisión y dejar que el agente materialice y ejecute un plan Lite directamente.',
    longHorizonLabel: 'Trabajo de largo horizonte — Full, guided',
    longHorizonBody:
      'Dependencias reales entre tareas, o mayor riesgo: revisar el plan propuesto antes de que empiece cualquier trabajo de producto, promoverlo a Full si el objetivo resulta necesitarlo, luego ejecutar y reanudar entre sesiones según se necesite.',
    ...
    endpoints: [
      ...
      },
      {
        ...
        description:
          'Catálogo versionado de la familia v1: rutas de endpoints, versión del sitio y enlaces a la especificación.',
      },
      {
        method: 'GET',
        path: '/api/v1/sections.json',
        description:
          'El mapa del sitio como JSON tipado — nombre, ruta y descripción por sección.',
      },
      {
        method: 'GET',
        path: '/api/v1/pages.json',
        description:
          'Cada endpoint de Markdown en cada idioma, agrupados por código de idioma.',
      },
      {
        method: 'GET',
        path: '/api/v1/health.json',
        description:
          'Marcador de estado versionado — el espejo v1 de /api/health.json.',
      },
      {
        method: 'POST',
        path: '/api/mcp',
    ...
    versioningTitle: 'Versionado y deprecación',
    versioningBody:
    ...
    rateLimitsTitle: 'Límites de peticiones',
    rateLimitsBody:
    ...
    cliCodeLabel: 'Terminal — CLI de skills y CLI oficial',
    cliNote:
      'La skill se vendoriza en .agents/skills/deepworkplan/ dentro de tu repositorio, de modo que cada agente que toca el repositorio comparte la misma metodología. La CLI oficial deepworkplan — un cliente sin dependencias sobre esta misma API (init, sections, read, open, mcp) — está preparada para npm y vive en el directorio cli/ del repositorio del sitio hasta su publicación.',
