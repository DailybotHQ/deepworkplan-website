...
export const ko: SiteTranslations = {
  ...
  home: {
    ...
    pitch: {
      kicker: '문제와 해답',
      problem:
        'AI 코딩 에이전트는 짧은 작업에서는 뛰어나지만, 긴 작업에서는 — 마이그레이션, 새 서브시스템, 대규모 리팩터링 — 표류합니다: 컨텍스트가 차고, 결정이 사라지고, 수 시간짜리 작업이 한가운데에서 멈춥니다.',
      answer:
        'Deep Work Plan은 스펙 주도 개발로 답합니다. 지속되는 계획, 원자적 작업, 그리고 에이전트가 통과해야 하는 검증 게이트. 작업은 검증 가능한 상태로 유지되며 — 어떤 에이전트든 세션 간에 재개할 수 있습니다.',
      illustrationAlt:
        '단일 항로로 위험을 안전하게 통과하는 해도 판화 — 계획이 작업을 올바른 항로로 유지하는 방향성을 표현합니다.',
    },
    story: {
      act1: {
        kicker: '방법론 · 제1막',
