...
- **Do** treat `.agents/skills/deepworkplan/` as repo-adapted: changes there must be intentional and reviewed. Prefer contributing reusable improvements upstream in `DailybotHQ/deepworkplan-skill`, then re-adapting this copy deliberately — never rely on the release dogfood step to pull it in.
  ...
  **Current vendored provenance (2026-09-11, v4.0.2 release):** this copy mirrors the upstream `main` branch at tag `v4.0.2` with **zero local adaptations**, installed with `npx --yes skills add DailybotHQ/deepworkplan-skill@v4.0.2 --skill deepworkplan --force -y`. Relative to the v4.0.0 baseline it carries two patches: the conformance checker was repaired — plan checks moved out of `verify/conformance.sh` into `verify/plan_contract.py` so Lite, Full and pre-v2 plans are judged by one implementation, and **legacy acceptance is restored**, so plans whose `state.json` predates the v2 schema are read against the shape they were authored under (`DWP_SPECIFICATION.md` §6.5) rather than rejected. That matters here: 6 of this repo's 10 plans under `.dwp/plans/` still carry the v1 state layer. The second patch repins the documented AI Diff Reviewer install to `@v2.0.1`, matching the addon vendored alongside it — the earlier gap noted here is closed. The v4.0.0 baseline is otherwise unchanged: the Lite-first plan lifecycle on DWP standard 2.4.0, `.dwp/drafts/` removed entirely (`create` materializes the plan directly), and `spec/LITE_PLANS.md` plus the v2 manifest/state JSON Schemas. This repository keeps the AI Diff Reviewer local-only; it does not ship an AI Reviewer CI workflow. The two addon skills (dailybot, ai-diff-reviewer) remain release-auto-refreshed.
...
DWP standard: 2.4.0 (onboarded 2026-09-11; skill 4.0.2)
