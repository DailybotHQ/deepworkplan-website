...
- **Do** treat `.agents/skills/deepworkplan/` as repo-adapted: changes there must be intentional and reviewed. Prefer contributing reusable improvements upstream in `DailybotHQ/deepworkplan-skill`, then re-adapting this copy deliberately — never rely on the release dogfood step to pull it in.
  ...
  **Current vendored provenance (2026-09-12):** this copy mirrors the upstream branch `fix/methodology-final-hardening` at commit `199ec1b` — the reviewed final-hardening work, **ahead of the last published tag `v4.0.3` and not yet an upstream release** — synced deliberately (file-by-file, `diff -r`-identical) rather than via `npx skills add`, because there is no tag to install. Its `skills-lock.json` content hash (`19d767bc…`) equals the skill repo's own dogfood lock at the same commit, so both copies provably carry the same bytes. Relative to `v4.0.3` it carries the final hardening: honest conformance exits (`UNVERIFIED`/2 without Python 3.9+, repaired bold-label field parsing), the Lite approval/promotion-recovery contracts, orchestrator handoff hardening (DWP_DIR isolation, mode-aware readiness, template checks that execute for real), the cross-addon consent matrix (AI Diff Reviewer required-local/CI-explicit; design-system mandatory-offer, acceptance-gated — never default-on; dependency-upgrade near-default/inert; devcontainer + dailybot explicit opt-ins), long-plan machinery (Stage Gates, enriched Plan Variables — optional, never Lite), the always-declared parallelization decision, Context enforcement (plan-level Goal+Context pair; task-level Context for v2+ tasks not yet completed — completed records stay as authored), the DWP standard **4.0.0** alignment (2.x accepted as historical; schema URLs stay v2; this repo's own plans authored at 2.4.0 remain valid as historical), the new `upgrade` sub-skill backing `/dwp-upgrade`, and the author sub-skill hardening (frontmatter field reference with per-harness opt-ins, first-class thin-delegator command template) together with the plan-local `analysis_results/` contract (every plan artifact lives under the plan's own folder, never the repository root). No local adaptation has been applied on top — this is the upstream branch verbatim; when the upstream PR merges and releases, the copy can be re-vendored from the tag with `npx --yes skills add DailybotHQ/deepworkplan-skill@vX.Y.Z --skill deepworkplan --force -y` and this paragraph re-stamped. This repository keeps the AI Diff Reviewer local-only; it does not ship an AI Reviewer CI workflow. The two addon skills (dailybot, ai-diff-reviewer) remain release-auto-refreshed.
...
DWP standard: 2.4.0 (onboarded 2026-09-11; skill 4.0.3)
