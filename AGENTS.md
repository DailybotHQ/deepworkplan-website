...
- **Do** treat `.agents/skills/deepworkplan/` as repo-adapted: changes there must be intentional and reviewed. Prefer contributing reusable improvements upstream in `DailybotHQ/deepworkplan-skill`, then re-adapting this copy deliberately — never rely on the release dogfood step to pull it in.
  ...
  **Current vendored provenance (2026-09-13; re-adapted 2026-09-17):** this copy's base is the released upstream tag **`v5.3.0`** (`ab9efd7`), superseding `v5.2.0`, installed via the documented command `npx --yes skills add DailybotHQ/deepworkplan-skill@v5.3.0 --skill deepworkplan --force -y`. The known container mount-race again struck one **non-canonical** fanout target of the CLI's multi-agent write (the "Sarvam Code" mirror), but that run the canonical `.agents/skills/deepworkplan/` write completed and was verified `diff -rq` byte-identical against the tag's canonical `skills/deepworkplan/` tree, with `skills-lock.json` updated by the CLI itself (hash `49a77d4f…`). Relative to `v5.2.0`, `v5.3.0` is the upstream **"v5 reliability closure"** release (PR #44): twelve tasks enforcing the v5 lifecycle guarantees the methodology already promised — state cannot silently contradict the Markdown (guarded writer + read-only checker), a task cannot close on evidence that admits non-execution (five evidence states in `state_contract.py`), completion is a recoverable transaction with a receipt (`finalize_plan.py` + `FINALIZATION.json`), evidence pointers stay resolvable after a handoff, an interrupted plan resumes at its exact boundary, and the docs match the shipped tree rather than prose claims — each backed by a regression scenario, with **thirteen defects found by fresh-agent acceptance runs** (frozen protocol/oracles, fault injection) folded back into the suite (324 → 398 cases). Standard remains aligned to **5.0.0** (no schema-line change).

> ⚠️ **This copy is no longer byte-identical to `v5.3.0`.** On **2026-09-17** it received a deliberate, reviewed **re-adaptation ahead of an upstream release**, mirroring `DailybotHQ/deepworkplan-skill` branch `feat/ai-diff-reviewer-v2.3.0-addon` (through commit **`c0c8986`**) — twelve files under `skills/deepworkplan/`, copied verbatim and verified with a full `diff -r` against that branch's tree (empty output). It carries three changes: (a) the **AI Diff Reviewer addon pin moves `v2.0.1` → `v2.3.0`** and the addon SPEC names an **incomplete review** — a run that starts and exits without writing findings — as a state distinct from a clean pass, so a Final Review cannot close on a review that never happened; and (b) `spec/DWP_SPECIFICATION.md` §5 now states normatively that **a plan's temporary and analysis output MUST live in that plan's own `analysis_results/`, never the repository root**, with `execute` and `create` referencing it; and (c) `spec/DOCUMENTATION_STANDARD.md` §7 scopes its `tmp/` guidance to **non-plan** scratch, resolving a contradiction the local review caught — both documents used the word "temporary" for opposite destinations, so an agent reading only the documentation standard would have written a plan's gate log to `tmp/`.
>
> **Consequence for the next upgrade — read before installing any tag.** A future `npx --yes skills add DailybotHQ/deepworkplan-skill@<tag> --skill deepworkplan --force -y` **overwrites this divergence wholesale**. It is only safe to lose once upstream has *released* a tag containing `62a6a27`; until then the re-adaptation must be re-applied after any reinstall. `skills-lock.json`'s `deepworkplan` entry deliberately still records the hash of the **`v5.3.0` source install** (`49a77d4f…`) and was **not** recomputed: that field records provenance of what was installed, not a fingerprint of the working tree, and hand-writing a hash there would assert a verification that never ran. This paragraph is the record of the delta.

Local adaptation therefore remains the command kit (seven `dwp-*` delegators incl. `/dwp-verify`) and this provenance protocol, **plus the 2026-09-17 re-adaptation above**; future updates follow the same explicit, reviewed path: install the new tag, re-apply any divergence not yet upstream, and re-stamp this paragraph. This repository keeps the AI Diff Reviewer local-only; it does not ship an AI Reviewer CI workflow. The two addon skills (dailybot, ai-diff-reviewer) remain release-auto-refreshed — `ai-diff-reviewer` was refreshed to **v2.3.0** on 2026-09-17 (lock hash `903e3868…`, verified `diff -rq` against the published tag).
