...
- **Do** treat `.agents/skills/deepworkplan/` as repo-adapted: changes there must be intentional and reviewed. Prefer contributing reusable improvements upstream in `DailybotHQ/deepworkplan-skill`, then re-adapting this copy deliberately — never rely on the release dogfood step to pull it in.
  ...
  **Current vendored provenance (2026-09-11, v4.0.1 release):** this copy mirrors the upstream `main` branch at tag `v4.0.1` (commit `9ee49a1`) with **zero local adaptations**, installed with `npx --yes skills add DailybotHQ/deepworkplan-skill@v4.0.1 --skill deepworkplan --force -y`. v4.0.1 is a patch over v4.0.0 that repairs the conformance checker: the plan checks moved out of `verify/conformance.sh` (925 -> 315 lines) into a new `verify/plan_contract.py`, so Lite, Full and pre-v2 plans are judged by one implementation, and **legacy acceptance is restored** — plans whose `state.json` predates the v2 schema are read against the shape they were authored under (`DWP_SPECIFICATION.md` §6.5) instead of being rejected. That matters here: 6 of this repo's 10 plans under `.dwp/plans/` still carry the v1 state layer. The v4.0.0 baseline is unchanged: the Lite-first plan lifecycle on DWP standard 2.4.0, `.dwp/drafts/` removed entirely (`create` materializes the plan directly), and `spec/LITE_PLANS.md` plus the v2 manifest/state JSON Schemas. This repository keeps the AI Diff Reviewer local-only; it does not ship an AI Reviewer CI workflow. The two addon skills (dailybot, ai-diff-reviewer) remain release-auto-refreshed. **Known gap:** v4.0.1 still documents the AI Diff Reviewer install as `@v2.0.0` while the vendored addon here is `2.0.1`; upstream [PR #39](https://github.com/DailybotHQ/deepworkplan-skill/pull/39) repins it and adds a CI gate, so re-vendor once that lands.
