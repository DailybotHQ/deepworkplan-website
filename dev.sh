...
while [ $# -gt 0 ]; do
  ...
  # and must reach it verbatim: `dev.sh exec dwpwebsitevscode pnpm run dev --host`
  # has to pass --host to pnpm, not have dev.sh reject it as its own unknown flag.
...
# The compose file may declare services beyond runServices (DWP ships one dev
# service today, but on-demand runners are exactly how compose files grow), and
# with the flag, starting the declared services alone would delete a container
# someone is using. Compose prints a warning recommending the flag — in this
# project that warning is an instruction to destroy containers the user asked
# for.
dc() {
  resolve_compose_bin
  compose_args
...
file_mode() {
  # GNU stat exits 0 on `stat -f` but prints FILESYSTEM info (here: empty), so
  # a plain `|| stat -c` fallback never runs on Linux and the caller sees "".
  # Probe the flavour once per call instead: --version exists on GNU only.
...
cmd_build() {
  ...
  if [ "$NO_CACHE" -eq 1 ]; then
    note "building ${SERVICES[*]} (--no-cache)"
...
# Rebuild images and replace running containers in one step.
# Equivalent to: bash dev.sh build && bash dev.sh up --recreate
# Use after Dockerfile / entrypoint / package install changes (e.g. PATH
# profile.d layers, new CLIs). Named volumes are kept.
cmd_rebuild() {
  fast_check
  write_overlay
  selected_services
  if [ "$NO_CACHE" -eq 1 ]; then
    note "rebuilding images for ${SERVICES[*]} (--no-cache, project $PROJECT)"
    dc build --no-cache "${SERVICES[@]}"
  else
    note "rebuilding images for ${SERVICES[*]} (project $PROJECT)"
    dc build "${SERVICES[@]}"
  fi
  note "recreating ${SERVICES[*]} with the new images"
  dc up -d --force-recreate "${SERVICES[@]}"
  note "rebuild done — named volumes were kept; open a new shell/pane so PATH reloads"
}
...
Flags
  ...
  --no-cache            with build/rebuild, pass --no-cache to compose build
...
Rebuild after Dockerfile / PATH / CLI changes:
  bash dev.sh rebuild
  bash dev.sh rebuild --no-cache
USAGE
}
