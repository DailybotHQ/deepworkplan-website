...
- **i18n:** Multilingual (17 active languages: en, es, pt, zh, ja, de, fr, ko, ru, it, tr, id, vi, hi, pl, uk, th — derived from `src/lib/translations/*.ts` via `getActiveLanguages()`) with centralized config in `src/lib/i18n.ts`, shared page components in `src/components/pages/`, one default-language wrapper in `src/pages/`, and one dynamic `src/pages/[lang]/<slug>.astro` wrapper that covers every non-default language
...
- [DeepWorkPlan skill (dwp-* flows)](../skills/deepworkplan/SKILL.md) — backs `/dwp-create`, `/dwp-execute`, `/dwp-refine`, `/dwp-resume`, `/dwp-status`, `/dwp-upgrade`
...
- [Design-system add-on](../skills/deepworkplan/addons/design-system/SKILL.md) — opt-in, frontend-scoped add-on shipped with the `deepworkplan` skill; generates a `docs/DESIGN.md` (indexed from `AGENTS.md`) from the repo's real design tokens so agents produce on-brand UI; a detected design system makes the offer mandatory while installation stays acceptance-gated (the visual profile is strongly recommended when detected, never default-on). This repo dogfoods it: see [docs/DESIGN.md](../../docs/DESIGN.md)
...
- [AI Diff Reviewer add-on](../skills/deepworkplan/addons/ai-diff-reviewer/SKILL.md) — structured-review add-on providing the required local review; this repo dogfoods **Flow A** (local-only): vendored `.agents/skills/ai-diff-reviewer/` (auto-refreshed on website release) plus `.review/extension.md`, run inside every DWP Final Review's security pass. This repo does not ship the CI Action — Flow B (`pr-review.yml`) is an explicit opt-in offered elsewhere, never installed unrequested (see AGENTS.md §Local AI Diff Reviewer)
