...
├── llms.txt                           # LLM-readable site summary (incl. When-to-use + Developer & Agent Resources)
├── llms-full.txt                      # LLM-readable full content
├── robots.txt                         # Search engine crawling rules (/api/ allowed, /internal/ disallowed)
├── site.webmanifest                   # PWA manifest
├── openapi.json                       # OpenAPI 3.1 spec of the agent API (version stamped by prebuild)
├── auth.md                            # Zero-auth agent access declaration
├── _headers / _redirects              # Cloudflare Pages config
├── api/
│   └── health.json                    # Static health marker (the MCP endpoint /api/mcp is a Pages Function, not a file)
├── .well-known/                       # Agent/MCP discovery files
│   ├── ai-catalog.json                # ARD capability manifest (agentmap)
│   ├── api-catalog                    # RFC 9727 linkset catalog
│   ├── mcp.json                       # MCP manifest (Streamable HTTP transport)
│   ├── mcp/server-card.json           # MCP server card
│   ├── agent-skills/index.json        # Generated by scripts/generate-agent-skills-index.mjs (prebuild)
│   ├── security.txt                   # RFC 9116 security contact
│   ├── dwp.json / dwp-trust.json      # DWP repo descriptor + provenance
│   └── oauth-authorization-server / oauth-protected-resource  # Zero-auth stubs
├── schema/                            # JSON Schemas (plan-manifest, plan-state, repo-descriptor)
...
| `openapi.json` | Machine-readable API description (OpenAPI 3.1; keep in sync with the agent surface — `pnpm run test` validates its structure) |
| `auth.md` | Declares the zero-auth agent access contract |
| `api/health.json` | Static health marker for the agent API |
| `_headers` | Cloudflare Pages headers config (applies only to static assets, never to Pages Functions) |
| `_redirects` | Cloudflare Pages redirect rules — real, single-hop server-side 301s (e.g. `/init` → `/quickstart`). Generated by `scripts/generate-redirects.mjs` (prebuild) from `src/lib/redirect-map.ts` + `src/lib/language-codes.ts` — do not hand-edit. This is what agents and crawlers actually follow; Astro's `redirects` config (`astro.config.mjs`) additionally renders an HTML meta-refresh page for the same routes as a client-side fallback only |
| `.well-known/` | Agent skills, ARD catalog, MCP manifest + server card, and OAuth discovery files |
| `schema/` | JSON Schemas for DWP plan manifests, plan state, and repo descriptors |
