...
The site currently has a single author (the Deep Work Plan team, slug `sergio-florez`), but the system is designed to scale to multiple contributors. The default author for any post without an explicit `author:` field is `sergio-florez`.
...
social:
  ...
  github: https://github.com/DailybotHQ
  instagram: https://www.deepworkplan.com
...
```jsonc
{
  ...
  "author": {
    ...
    "image": "https://deepworkplan.com/images/authors/sergio-florez.webp"
  }
}
```

When the slug doesn't resolve, `name` falls back to `'the Deep Work Plan team'` and `image` to `/images/og-default.png`.
